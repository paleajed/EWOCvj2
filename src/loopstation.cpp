...
void LoopStation::handle() {
    ...
    this->upscrbox->upvtxtoscr();
    this->downscrbox->upvtxtoscr();
    if (!mainprogram->binsroom && !mainprogram->styleroom && !mainprogram->genroom) render_text("Loopstation", white, elements[0]->recbut->box->vtxcoords->x1 + 0.015f,
...
void LoopStationElement::handle() {
    if (!mainprogram->binsroom && !mainprogram->styleroom && !mainprogram->genroom && this->pos >= this->lpst->scrpos && this->pos < this->lpst->scrpos + 8){
        this->visualize();
        this->mouse_handle();
        for (int i = 0; i < 2; i++) {
