...
void handle_midi(std::vector<Layer*> &lvec, int deck, int midi0, int midi1, int midi2, std::string midiport) {
	...
	for (int j = 0; j < lvec.size(); j++) {
		...
		if (1) {
		    ...
		    if (midi0 == laymidi->beatthres->midi0 && midi1 == laymidi->beatthres->midi1 && midiport == laymidi->beatthres->midiport) {
		        mainprogram->beatthres->value = (float) midi2 / 127.0f;
		        mainmix->midi2 = midi2;
		        par = mainprogram->beatthres;
		        mainprogram->beatthres->midistarttime = std::chrono::system_clock::now();
		        mainprogram->beatthres->midistarted = true;
		    }
		    if (but) {
                for (int i = 0; i < loopstation->elements.size(); i++) {
                    if (loopstation->elements[i]->recbut->value) {
  	    ...
  	    if (mainprogram->configcatmidi == 0) {
            ...
            switch (mainprogram->tmlearn) {
                case TM_NONE:
                    ...
                    else if (lm->beatthres->midi0 == midi0 && lm->beatthres->midi1 == midi1 &&
                            lm->beatthres->midiport == midiport)
                        mainprogram->tmchoice = TM_BEATTHRES;
	...
	if (node->type == EFFECT) {
		...
		if (effect->onoffbutton->value) {
            ...
            if (lasteffect)
            {
                ...
                swidth = sw * sc;
                sheight = sh * sc;
                // Full-viewport approach: AR + user scale/shift via tc (no viewport growth)
                float xss_safe = (lay->xss > 0.001f) ? lay->xss : 1.0f;
                float yss_safe = (lay->yss > 0.001f) ? lay->yss : 1.0f;
                tc_scale_eff  = 1.0f / (xss_safe * sc);
                tc_scaley_eff = 1.0f / (yss_safe * sc);
                tc_dx_eff     = 2.0f * sx / sc;
                tc_dy_eff     = 2.0f * sy / sc;
            ...
            if (effect->masktex != -1) {
                // enable masking of effect influence: masktex used as grayscale mask
                mainprogram->uniformCache->setBool("usemask", true);
                umask = true;
                mainprogram->uniformCache->setSampler("Sampler3", 3);
		    ...
		    if (effect->ffglnr != -1 && effect->onoffbutton->value) {
                ...
                if (!lay->onhold) {
                    ...
                    if (effect->type == MIRROR) {
                        // MIRROR: step 1 applies shift+mask (passthrough), step 2 applies the mirror effect
                        mainprogram->uniformCache->setInt("fxid", 42);
                        mainprogram->uniformCache->setInt("interm", 2);
                        mainprogram->uniformCache->setBool("usemask", umask);
                        draw_direct(nullptr, black, -1.0f, 1.0f, 2.0f, -2.0f, lasteffect ? tc_dx_eff : 0.0f, lasteffect ? tc_dy_eff : 0.0f, lasteffect ? tc_scale_eff : 1.0f, 1.0f, 0, prevfbotex, 0, 0, false, false, lasteffect ? tc_scaley_eff : 1.0f);
                    } else {
                        // Non-MIRROR: step 1 applies effect only (no shift, no mask)
        ...
        lay->scw = scw;
        lay->sch = sch;
        float tc_dx = 2.0f * sx / sc;
        float tc_dy = 2.0f * sy / sc;
        // ISF sources generate at output resolution - no aspect ratio correction needed
        if (lay->isfsourcenr != -1) {
            xs = 1.0f;
            ys = 1.0f;
        }
        // Full-viewport approach: AR + scale + shift via tc (no viewport growth, no GL_MAX_VIEWPORT_DIMS issue)
        float xs_safe = (xs > 0.001f) ? xs : 1.0f;
        float ys_safe = (ys > 0.001f) ? ys : 1.0f;
        float src_tc_scale_x = effectspresent ? 1.0f : (1.0f / (xs_safe * sc));
        float src_tc_scale_y = effectspresent ? 1.0f : (1.0f / (ys_safe * sc));
        float src_tc_dx = effectspresent ? 0.0f : tc_dx;
        float src_tc_dy = effectspresent ? 0.0f : tc_dy;
        ...
        if (lay->ndisource != nullptr) {
            ...
            glClear(GL_COLOR_BUFFER_BIT);
            mainprogram->uniformCache->setInt("interm", 4);
            draw_direct(nullptr, black, -1.0f, 1.0f, 2.0f, -2.0f, src_tc_dx, src_tc_dy, src_tc_scale_x, op, 0, lay->ndiintex.getTextureID(), 0, 0, false, false, src_tc_scale_y);
        }
            ...
            if (!lay->onhold && lay->filename != "") {
                if (lay->changeinit == 2) {
                    draw_direct(nullptr, black, -1.0f, 1.0f, 2.0f, -2.0f, src_tc_dx, src_tc_dy, src_tc_scale_x, op, 0, lay->texture, 0, 0, false, false, src_tc_scale_y);
                } else {
                    draw_direct(nullptr, black, -1.0f, 1.0f, 2.0f, -2.0f, src_tc_dx, src_tc_dy, src_tc_scale_x, op, 0, lay->oldtexture, 0, 0, false, false, src_tc_scale_y);
                }
            }
            mainprogram->uniformCache->setInt("interm", 0);
        }
                ...
                if (1) {
                    if (1) {
                        if (bnode->ffglmixernr != -1) {
                            ...
                            if (bnode->tempfbo == (GLuint)-1) {
                                GLuint rettex = mainprogram->grab_from_texpool(mainprogram->ow[stage], mainprogram->oh[stage], GL_RGBA8);
                                if (rettex != (GLuint)-1) {
                                    bnode->tempfbotex = rettex;
                                } else {
                                    glGenTextures(1, &(bnode->tempfbotex));
                                ...
                                if (retfbo != (GLuint)-1) {
                                    bnode->tempfbo = retfbo;
                                } else {
                                    glGenFramebuffers(1, &(bnode->tempfbo));
                                }
                                glBindFramebuffer(GL_FRAMEBUFFER, bnode->tempfbo);
                                glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, bnode->tempfbotex, 0);
                            }
...
void the_loop() {
    ...
    // ComfyUI throttle: if fps was below target last frame and generation is running,
    // suspend the ComfyUI process now so our OpenGL render gets exclusive GPU access.
    // It will be resumed just before SDL_Delay so it can progress during our idle time.
    bool comfyGenRunning = mainvideogenroom && mainvideogenroom->comfyManager &&
                            mainvideogenroom->comfyManager->isGenerating();
    bool comfyThrottleNow = comfyUIThrottleEnabled && comfyGenRunning &&
                             mainmix->fps[24] != 0 &&
                             (float)mainmix->rate < mainprogram->project->targetframerate * 0.95f;
    if (comfyThrottleNow) {
        ...
        for (int i = 0; i <= done; i++)
        {
            ...
            if (tempmap->size()) {
                ...
                if (i > 7) {
                    ...
                    // For main-layer swaps: update mask->parentlayer and effect mask
                    // parenteffect/parentlayer from old_layer to new_layer now that the swap
                    // is actually happening. This was intentionally deferred from transfer()
                    // to avoid leaving old_layer without a mask in the frame before the swap.
                    for (std::vector<Layer*>& lv_entry : *tempmap) {
                        if (lv_entry[0] && lv_entry[1]) {
                            Layer* new_lay = lv_entry[1];
                ...
                for (int p = 0; p < mainmix->currlays[!mainprogram->prevmodus].size(); p++) {
                    ...
                    if (!mainprogram->prevmodus == lvec[0]->comp && cl->deck == i % 2) {
                        Layer *newcl;
                        if (cl->pos < lvec.size())
                        {
                            newcl = lvec[cl->pos];
            ...
            tempmap->clear();
            if (i > 7) {
                mainmix->neweffmasks[i - 8].clear();
            }
            else if (i > 3) {
                mainmix->newmasks[i - 4].clear();
            }
            else {
                mainmix->newlrs[i].clear();
            }
        }
    }
            ...
            if (!binel->encoding && binel->encodingend != "") {    // exchange original for hap version everywhere (layerstacks, shelves, bins)
                for (int k = 0; k < 4; k++) {
                    for (int m = 0; m < mainmix->layers[k].size(); m++) {
                        ...
                        bool bukmb = lay->keepmaskbut->value;
                        lay->keepeffbut->value = 1;
                        lay->keepmaskbut->value = 1;
                        lay->dontcloseeffs = 2;
                        lay->dontclosemasks = 1;
                        lay->tagged = true;
	...
	//}
		...
		if (mainprogram->prevmodus) {
			...
			// when performing
            if (!mainmix->editedmask[1][0]) {
                for (int i = 0; i < mainmix->layers[2].size(); i++) {
                    Layer *testlay = mainmix->layers[2][i];
                    testlay->display();
                }
            }
            if (!mainmix->editedmask[1][1]) {
                for (int i = 0; i < mainmix->layers[3].size(); i++) {
                    Layer *testlay = mainmix->layers[3][i];
                    testlay->display();
                }
        ...
        for (int c = 0; c < 2; c++) {
            for (int d = 0; d < 2; d++) {
                if (mainmix->editedmaskeff[c][d]) {
                    auto masks = mainmix->editedmaskeff[c][d]->masks;
                    for (auto masklay : masks) {
                        masklay->display();
                    }
                    ...
                    auto masks = mainmix->editedmask[c][d]->masks;
                    for (auto masklay : masks) {
                        masklay->display();
                    }
		...
		if (mainprogram->dragbinel && !mainmix->moving) {
			if (mainprogram->dragbinel->type != ELEM_DECK && mainprogram->dragbinel->type != ELEM_MIX) {
				if (mainprogram->prevmodus) {
                    if (mainmix->editedmaskeff[0][0]) {
                        drag_into_layerstack(mainmix->editedmaskeff[0][0]->masks, 0);
                    }
                    else if (mainmix->editedmask[0][0]) {
                        drag_into_layerstack(mainmix->editedmask[0][0]->masks, 0);
                    }
                    else {
                        drag_into_layerstack(mainmix->layers[0], 0);
                    }
                    if (mainmix->editedmaskeff[0][1]) {
                        drag_into_layerstack(mainmix->editedmaskeff[0][1]->masks, 1);
                    }
                    else if (mainmix->editedmask[0][1]) {
                        drag_into_layerstack(mainmix->editedmask[0][1]->masks, 1);
                    }
                    else {
                    ...
                    if (mainmix->editedmaskeff[1][0]) {
                        drag_into_layerstack(mainmix->editedmaskeff[1][0]->masks, 0);
                    }
                    else if (mainmix->editedmask[1][0]) {
                        drag_into_layerstack(mainmix->editedmask[1][0]->masks, 0);
                    }
                    else {
                        drag_into_layerstack(mainmix->layers[2], 0);
                    }
                    if (mainmix->editedmaskeff[1][1]) {
                        drag_into_layerstack(mainmix->editedmaskeff[1][1]->masks, 1);
                    }
                    else if (mainmix->editedmask[1][1]) {
                        drag_into_layerstack(mainmix->editedmask[1][1]->masks, 1);
                    }
                    else {
    ...
    // Resume ComfyUI before sleeping so it can use our idle GPU time.
...
void write_genmidi(ostream& wfile, LayMidi *lm) {
    ...
    wfile << "BEATTHRES\n";
    wfile << std::to_string(lm->beatthres->midi0);
    wfile << "\n";
    wfile << std::to_string(lm->beatthres->midi1);
    wfile << "\n";
    wfile << lm->beatthres->midiport;
    wfile << "\n";
...
void open_genmidis(std::string path) {
	...
	while (safegetline(rfile, istring)) {
	    ...
	    if (istring == "BEATTHRES") {
	        safegetline(rfile, istring);
	        lm->beatthres->midi0 = std::stoi(istring);
	        safegetline(rfile, istring);
	        lm->beatthres->midi1 = std::stoi(istring);
	        safegetline(rfile, istring);
	        lm->beatthres->midiport= istring;
	        lm->beatthres->register_midi();
	    }
	    if (istring == "PLAY") {
            safegetline(rfile, istring);
