...
float darkred1[] = { 0.4f, 0.0f, 0.0f, 1.0f };
...
bool isvideo(const std::string &path) {
    ...
    if (r < 0) return false;
    if (avformat_find_stream_info(test, nullptr) < 0) return false;
    return true;
}
...
void handle_midi(LayMidi *laymidi, int deck, int midi0, int midi1, int midi2, std::string &midiport) {
	...
	for (int j = 0; j < lvec.size(); j++) {
		if (lvec[j]->genmidibut->value == genmidideck) {
			if (midi0 == laymidi->play[0] && midi1 == laymidi->play[1] && midi2 == 127 && midiport == laymidi->playstr) {
				lvec[j]->playbut->value = !lvec[j]->playbut->value;
			}
			if (midi0 == laymidi->pausestop[0] && midi1 == laymidi->pausestop[1] && midi2 == 127 && midiport == laymidi->pausestopstr) {
				lvec[j]->playbut->value = false;
			}
			if (midi0 == laymidi->bounce[0] && midi1 == laymidi->bounce[1] && midi2 == 127 && midiport == laymidi->bouncestr) {
				lvec[j]->bouncebut->value = !lvec[j]->bouncebut->value;
			}
			if (midi0 == laymidi->scratch[0] && midi1 == laymidi->scratch[1] && midiport == laymidi->scratchstr) {
				lvec[j]->scratch = ((float)midi2 - 64.0f) / 4.0f;
			}
			if (midi0 == laymidi->frforw[0] && midi1 == laymidi->frforw[1] && midi2 == 127 && midiport == laymidi->frforwstr) {
				lvec[j]->frame += 1;
				if (lvec[j]->frame >= lvec[j]->numf) lvec[j]->frame = 0;
			}
			if (midi0 == laymidi->frbackw[0] && midi1 == laymidi->frbackw[1] && midi2 == 127 && midiport == laymidi->frbackwstr) {
				lvec[j]->frame -= 1;
				if (lvec[j]->frame < 0) lvec[j]->frame = lvec[j]->numf - 1;
			}
			if (midi0 == laymidi->scratchtouch[0] && midi1 == laymidi->scratchtouch[1] && midi2 == 127 && midiport == laymidi->scratchtouchstr) {
				lvec[j]->scratchtouch = 1;
			}
			if (midi0 == laymidi->scratchtouch[0] && midi1 == laymidi->scratchtouch[1] && midi2 == 0 && midiport == laymidi->scratchtouchstr) {
				lvec[j]->scratchtouch = 0;
			}
			if (midi0 == laymidi->speed[0] && midi1 == laymidi->speed[1] && midiport == laymidi->speedstr) {
				int m2 = -(midi2 - 127);
				if (m2 >= 64.0f) {
			...
			if (midi0 == laymidi->speedzero[0] && midi1 == laymidi->speedzero[1] && midiport == laymidi->speedzerostr) {
				lvec[j]->speed->value = 1.0f;
			}
			if (midi0 == laymidi->opacity[0] && midi1 == laymidi->opacity[1] && midiport == laymidi->opacitystr) {
    ...
    if (mainprogram->waitmidi == 1) {
  	...
  	if (mainprogram->midipresets) {
		...
		switch (mainprogram->tmlearn) {
			case TM_NONE:
				if (lm->play[0] == midi0 && lm->play[1] == midi1 && lm->playstr == midiport) mainprogram->tmchoice = TM_PLAY;
				else if (lm->backw[0] == midi0 && lm->backw[1] == midi1 && lm->backwstr == midiport) mainprogram->tmchoice = TM_BACKW;
				else if (lm->bounce[0] == midi0 && lm->bounce[1] == midi1 && lm->bouncestr == midiport) mainprogram->tmchoice = TM_BOUNCE;
				else if (lm->frforw[0] == midi0 && lm->frforw[1] == midi1 && lm->frforwstr == midiport) mainprogram->tmchoice = TM_FRFORW;
				else if (lm->frbackw[0] == midi0 && lm->frbackw[1] == midi1 && lm->frbackwstr == midiport) mainprogram->tmchoice = TM_FRBACKW;
				else if (lm->speed[0] == midi0 && lm->speed[1] == midi1 && lm->speedstr == midiport) mainprogram->tmchoice = TM_SPEED;
				else if (lm->speedzero[0] == midi0 && lm->speedzero[1] == midi1 && lm->speedzerostr == midiport) mainprogram->tmchoice = TM_SPEEDZERO;
				else if (lm->opacity[0] == midi0 && lm->opacity[1] == midi1 && lm->opacitystr == midiport) mainprogram->tmchoice = TM_OPACITY;
				else if (lm->scratchtouch[0] == midi0 && lm->scratchtouch[1] == midi1 && lm->scratchtouchstr == midiport) mainprogram->tmchoice = TM_FREEZE;
				else if (lm->scratch[0] == midi0 && lm->scratch[1] == midi1 && lm->scratchstr == midiport) mainprogram->tmchoice = TM_SCRATCH;
  	...
  	if (mainmix->learn) {
  		if (midi0 == 176 && midi2 == 127 && mainmix->learnparam && mainmix->learnparam->sliding) {
			mainmix->learn = false;
...
IMPLEMENT */		}
		else if (midi0 == 144 && mainmix->learnbutton) {
			if (mainmix->midishelfstage == 1) {
				// start midi button
				mainmix->midishelfstage = 2;
				mainmix->midishelfstart = midi1;
 ...
 */		}
  	...
  	if (midi0 == 176 || midi0 == 144) {
		for (int i = 0; i < mainprogram->buttons.size(); i++) {
			...
			if (midi0 == but->midi[0] && midi1 == but->midi[1] && midiport == but->midiport) {
				mainmix->midi2 = midi2;
...
bool Layer::thread_vidopen() {
    ...
    if (this->ifmt) this->type = ELEM_LIVE;
...
std::vector<float> render_text(std::string text, float *textc, float x, float y, float sx, float sy, int smflag, bool display, bool vertical) {
	...
	if (smflag == 0) y -= 0.012f * (2070.0f / glob->h);
	if (smflag > 0) y -= 0.02f * (2070.0f / glob->h);
	GLuint texture;
	float wfac;
	float textw = 0.0f;
	float texth = 0.0f;
...
void Layer::load_frame() {
	...
	if (!this->liveinput && srclay->loaded > 2) {
		if ((!this->isclone)) {  // decresult contains new frame width, height, number of bitmaps && data
			...
			if ((this->vidformat == 188 || this->vidformat == 187) && srclay->video_dec_ctx) {
				...
				// HAP video layers
				if (srclay->decresult->compression == 187 || srclay->decresult->compression == 171) {
	...
	if (srclay->loaded < 4) srclay->loaded++;
...
void onestepfrom(bool stage, Node *node, Node *prevnode, GLuint prevfbotex, GLuint prevfbo) {
	...
	// #define BUFFER_OFFSET(i) ((char *)nullptr + (i))
		...
		if (lay->pos == 0) lay->blendnode->fbotex = lay->fbotex;
	}
...
void walk_nodes(bool stage) {
	...
	if (stage == 0) {
		...
		for (int i = 0; i < mainmix->layersA.size(); i++) {
			...
			if (!lay->liveinput && !lay->decresult->width && lay->filename != "") return;
		...
		for (int i = 0; i < mainmix->layersAcomp.size(); i++) {
			...
			if (!lay->liveinput && !lay->decresult->width && lay->filename != "" && lay->vidopen == false) return;
			if (lay->mutebut->value) mutes++;
			walk_forward(lay->node);
...
bool display_mix() {
	...
	GLint mixmode = glGetUniformLocation(mainprogram->ShaderProgram, "mixmode");
    if (mainprogram->prevmodus) {
        ((MixNode*)(mainprogram->nodesmain->mixnodes[0]))->outputbox = mainprogram->deckmonitor[0];
        ((MixNode*)(mainprogram->nodesmain->mixnodes[1]))->outputbox = mainprogram->deckmonitor[1];
        ((MixNode*)(mainprogram->nodesmain->mixnodes[2]))->outputbox = mainprogram->mainmonitor;
        ((MixNode*)(mainprogram->nodesmain->mixnodescomp[2]))->outputbox = mainprogram->outputmonitor;
    }
    else {
        ((MixNode*)(mainprogram->nodesmain->mixnodes[2]))->outputbox = mainprogram->outputmonitor;
        ((MixNode*)(mainprogram->nodesmain->mixnodescomp[0]))->outputbox = mainprogram->deckmonitor[0];
        ((MixNode*)(mainprogram->nodesmain->mixnodescomp[1]))->outputbox = mainprogram->deckmonitor[1];
        ((MixNode*)(mainprogram->nodesmain->mixnodescomp[2]))->outputbox = mainprogram->mainmonitor;
    }
	if (mainprogram->prevmodus) {
		if (mainmix->wipe[0] > -1) {
			...
			if (mainprogram->leftmousedown || mainprogram->middlemousedown) {
				...
				if (mainprogram->dragbinel) {
					...
					if (extstr != "") {
                        ...
                        boost::filesystem::copy_file(mainprogram->dragbinel->path, find_unused_filename("shelf_" + base, mainprogram->project->shelfdir, extstr));
		...
		if (box->in() || but->value) {
			if (mainprogram->menuactivation && !mainprogram->menuondisplay) {
				...
				if (((mainprogram->onscenemilli > 6000 || mainprogram->leftmouse) && !mainprogram->menuondisplay) || but->value) {
					...
					if (scene->deck == !bulay->deck) mainmix->currlay[!mainprogram->prevmodus] = lvec2[std::clamp(bulay->pos, 0, max)];
		...
		if (!mainprogram->menuondisplay && mainprogram->dragbinel) {
			for (int j = 0; j < 2; j++) {
				...
				for (int i = 0; i < lvec.size(); i++) {
					Layer* lay = lvec[i];
					bool comp = !mainprogram->prevmodus;
					if (lay->pos < mainmix->scenes[comp][j][mainmix->currscene[comp][j]]->scrollpos || lay->pos > mainmix->scenes[comp][j][mainmix->currscene[comp][j]]->scrollpos + 2) continue;
					Box* box = lay->node->vidbox;
					...
					if (box->scrcoords->y1 - box->scrcoords->h < mainprogram->my && mainprogram->my < box->scrcoords->y1) {
						if (box->scrcoords->x1 - thick + (i - mainmix->scenes[comp][j][mainmix->currscene[comp][j]]->scrollpos == 0) * thick < mainprogram->mx && mainprogram->mx < box->scrcoords->x1 + thick) {
							...
							mainprogram->leftmousedown = false;
							if (lay->pos == lvec.size() - 1 || lay->pos == mainmix->scenes[comp][j][mainmix->currscene[comp][j]]->scrollpos + 2) {
...
void Shelf::open_files_shelf() {
    ...
    elem->jpegpath = find_unused_filename(this->basepath, mainprogram->temppath, ".jpg");
...
int main(int argc, char* argv[]) {
   ...
   boost::filesystem::path p4{ homedir + "/.ewocvj2/temp" };
    ...
    mainprogram->dummywindow = SDL_CreateWindow("", 0, 0, 32, 32,
    ...
    //glc = SDL_GL_CreateContext(mainprogram->config_midipresetswindow);
    SDL_GL_MakeCurrent(mainprogram->config_midipresetswindow, glc);
    ...
    for (int m = 0; m < 2; m++) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            lay->closethread = true;
            mainmix->scenes[comp][m][mainmix->currscene[comp][m]]->nblayers.insert(
                    mainmix->scenes[comp][m][mainmix->currscene[comp][m]]->nblayers.begin(), lay);
            mainmix->scenes[comp][m][mainmix->currscene[comp][m]]->nbframes.insert(
                    mainmix->scenes[comp][m][mainmix->currscene[comp][m]]->nbframes.begin(), lay->frame);
            lay->clips.clear();
            mainmix->mousedeck = m;
            mainmix->do_save_deck(
                    mainprogram->temppath + "tempdeck_0" + std::to_string(m) + std::to_string(i) + ".deck", false,
                    false);
            mainmix->do_save_deck(
                    mainprogram->temppath + "tempdeck_1" + std::to_string(m) + std::to_string(i) + ".deck", false,
                    false);
            lvec.clear();
            lay->filename = "";
        }
    ...
    mainmix->copy_to_comp(mainmix->layersA, mainmix->layersAcomp, mainmix->layersB, mainmix->layersBcomp,
                          mainprogram->nodesmain->currpage->nodes, mainprogram->nodesmain->currpage->nodescomp,
                          mainprogram->nodesmain->mixnodescomp, true);
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_BORDER);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_BORDER);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, 3840, 2070, 0, GL_BGRA, GL_UNSIGNED_BYTE, (char *) ilGetData());
    ...
    while (!quit) {
        ...
        if (mainprogram->path != "") {
            ...
            mainprogram->path = "";
            mainprogram->pathto = "";
            if (!exists(mainprogram->currfilesdir)) mainprogram->currfilesdir = mainprogram->docpath + "elems/";
            if (!exists(mainprogram->currclipfilesdir)) mainprogram->currclipfilesdir = mainprogram->docpath + "elems/";
            if (!exists(mainprogram->currshelffilesdir))
                mainprogram->currshelffilesdir = mainprogram->docpath + "elems/";
            if (!exists(mainprogram->currbinfilesdir)) mainprogram->currbinfilesdir = mainprogram->docpath + "elems/";
            if (!exists(mainprogram->currstatedir)) mainprogram->currstatedir = mainprogram->docpath + "elems/";
        }
        ...
        while (SDL_PollEvent(&e)) {
                    ...
                    if (mainprogram->ctrl) {
                        if (e.key.keysym.sym == SDLK_s) {
                            ...
                            std::thread filereq(&Program::get_outname, mainprogram, "Save state file",
                                            ...
                                            mainprogram->currstatedir).generic_string());
                            filereq.detach();
                        }
            ...
            if (focus && SDL_GetMouseFocus() != mainprogram->prefwindow &&
            ...
            if (box->in()) {
                ...
                if (mainprogram->leftmouse) {
                    ...
                    mainprogram->get_outname("New project", "application/ewocvj2-project",
                                             boost::filesystem::absolute(reqdir + "/" + name + ".ewocvj").generic_string
                                             ());
                    if (mainprogram->path != "") {
                        SDL_GL_MakeCurrent(mainprogram->mainwindow, glc);
                        mainprogram->project->newp(mainprogram->path);
