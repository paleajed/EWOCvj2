...
bool Program::do_order_paths() {
    ...
    if (this->paths.size() > 18) {
        ...
        if (this->paths.size() - this->pathscroll < 18) this->pathscroll = this->paths.size() - 17;
    }
...
void Program::compile_effect_shaders() {
    std::cout << "Starting parallel compilation of 43 effect shaders..." << std::endl;
    ...
    if (!supportsAsync) {
        std::cout << "ARB_parallel_shader_compile not supported for effect shaders" << std::endl;
        // Initialize all to 0 as fallback (use main shader)
        for (int i = 0; i < 43; i++) {
            EffectShaderPrograms[i] = 0;
        }
        return;
    }

    // Load the main shader source
    char *VShaderSource;
    char *FShaderSource;
    unsigned long vlen = 0;
    unsigned long flen = 0;
    ...
    if (exists("./shader.vs")) strcpy (vshader, "./shader.vs");
        ...
        std::cerr << "Unable to find vertex shader shader.vs" << std::endl;
        return;
    }
    if (exists("./shader.fs")) strcpy (fshader, "./shader.fs");
    else {
        std::cerr << "Unable to find fragment shader shader.fs" << std::endl;
        return;
    }
    #else
    #ifdef POSIX
    std::string ddir (this->docpath);
    std::string vstr = appimagedir + "/usr/share/ewocvj2/shader.vs";
    std::string fstr = appimagedir + "/usr/share/ewocvj2/shader.fs";
    if (exists(vstr)) strcpy (vshader, vstr.c_str());
    else {
        std::cerr << "Unable to find vertex shader" << std::endl;
        return;
    }
    if (exists(fstr)) strcpy (fshader, fstr.c_str());
    else {
        std::cerr << "Unable to find fragment shader" << std::endl;
        return;
    }
    #endif
    #endif
    ...
    std::string vertexSource(VShaderSource);
    std::string fragmentSource(FShaderSource);

    // Find where main() starts in the fragment shader (around line 1556)
    size_t mainPos = fragmentSource.find("void main()");
    if (mainPos == std::string::npos) {
        std::cerr << "Could not find main() in fragment shader" << std::endl;
        free(VShaderSource);
        free(FShaderSource);
        free(vshader);
        free(fshader);
    ...
    // Find the effect processing section to remove (interm == 1 section with switch statement)
    size_t effectSectionStart = fragmentSource.find("if (interm == 1) {");
    size_t effectSectionEnd = std::string::npos;
    if (effectSectionStart != std::string::npos) {
        // Find the closing brace of the interm == 1 block (before "else if (mixmode > 0)")
        effectSectionEnd = fragmentSource.find("else if (mixmode > 0) {", effectSectionStart);
        if (effectSectionEnd != std::string::npos) {
            // Backtrack to find the closing brace
            size_t pos = effectSectionEnd - 1;
            while (pos > effectSectionStart && fragmentSource[pos] != '}') {
                pos--;
            }
            if (fragmentSource[pos] == '}') {
                effectSectionEnd = pos + 1; // Include the closing brace
            }
        }
    }

    // Extract the common code (everything before main())
    std::string commonCode = fragmentSource.substr(0, mainPos);

    // Extract the main() code without the effect section for the core shader
    std::string coreMainCode = fragmentSource.substr(mainPos);
    if (effectSectionStart != std::string::npos && effectSectionEnd != std::string::npos) {
        // Remove the effect processing section from core shader
        size_t sectionLen = effectSectionEnd - effectSectionStart;
        coreMainCode = fragmentSource.substr(mainPos, effectSectionStart - mainPos) +
                       fragmentSource.substr(effectSectionEnd);
    }

    // Effect names matching the fxid values in the switch statement
    const char* effectCalls[43] = {
        "blur(texco)",              // 0: BLUR
        "brightness(texcol)",       // 1: BRIGHTNESS
        "chromarotate(texcol)",     // 2: CHROMAROTATE
        "contrast(texcol)",         // 3: CONTRAST
        "dotf(texco)",              // 4: DOT
        "boxblur(texco)",           // 5: GLOW (uses boxblur with glowfac)
        "radialblur(texco)",        // 6: RADIALBLUR
        "saturation(texcol)",       // 7: SATURATION
        "texture(Sampler0, scale(texco))",  // 8: SCALE
        "swirl(texco)",             // 9: SWIRL
        "oldfilm(texco)",           // 10: OLDFILM
        "ripple()",                 // 11: RIPPLE
        "fisheye(texco)",           // 12: FISHEYE
        "treshold(texcol)",         // 13: TRESHOLD
        "texcol",                   // 14: STROBE (handled by uniform)
        "posterize(texcol)",        // 15: POSTERIZE
        "pixelate(texco)",          // 16: PIXELATE
        "crosshatch(texcol, texco)",// 17: CROSSHATCH
        "vec4(1.0 - texcol.r, 1.0 - texcol.g, 1.0 - texcol.b, 1.0)",  // 18: INVERT
        "rotate(texco)",            // 19: ROTATE
        "emboss(texco)",            // 20: EMBOSS
        "ascii(texco)",             // 21: ASCII
        "solarize(texco)",          // 22: SOLARIZE
        "vardot(texco)",            // 23: VARDOT
        "crt(texco)",               // 24: CRT
        "edgedetect(texco)",        // 25: EDGEDETECT
        "kaleidoscope(texco)",      // 26: KALEIDOSCOPE
        "halftone(texcol, texco)",  // 27: HTONE
        "cartoon(texco)",           // 28: CARTOON
        "cutoff(texco)",            // 29: CUTOFF
        "glitch(texco)",            // 30: GLITCH
        "colorize(texcol)",         // 31: COLORIZE
        "noise(texco)",             // 32: NOISE
        "gammamain(texco)",         // 33: GAMMA
        "thermal(texco)",           // 34: THERMAL
        "bokehmain(texco)",         // 35: BOKEH
        "sharpen(texco)",           // 36: SHARPEN
        "dither(texco)",            // 37: DITHER
        "flip(texco)",              // 38: FLIP
        "mirror(texco)",            // 39: MIRROR
        "boxblur(texco)",           // 40: BOXBLUR
        "chromastretch(texcol)",    // 41: CHROMASTRETCH
        "texcol"                    // 42: PASS (passthrough)
    };

    // Structure to hold shader compilation state
    struct EffectShaderBatch {
        int fxid;
    ...
    std::vector<EffectShaderBatch> batches;
    batches.reserve(43);

    // PHASE 1: Generate all shader variants
    std::cout << "Phase 1: Generating 43 effect shader variants..." << std::endl;
    for (int fxid = 0; fxid < 43; fxid++) {
        EffectShaderBatch batch;
        batch.fxid = fxid;
        std::cout << "DEBUG: Creating batch for fxid=" << fxid << std::endl;

        // Generate a minimal main() that only calls this specific effect
        std::string effectMain = R"(
void main()
{
    vec2 texco = TexCoord0;
    vec4 texcol = texture(Sampler0, texco);
    vec4 intcol = )";

        effectMain += effectCalls[fxid];
        effectMain += R"(;
    ...
    FragColor = vec4(intcol.rgb * drywet + (1.0f - drywet) * texcol.rgb, intcol.a * opacity);
        ...
        batch.fragmentSource = commonCode + effectMain;
        batches.push_back(std::move(batch));
        std::cout << "DEBUG: Pushed batch with fxid=" << batches.back().fxid << " (batch count now: " << batches.size() << ")" << std::endl;
    }

    // PHASE 2: Kick off ALL shader compilations (non-blocking) - INCLUDING core shader
    std::cout << "Phase 2: Starting compilation of " << batches.size() << " effect shaders + core shader in parallel..." << std::endl;
    ...
    // Also kick off core shader compilation in parallel
    GLuint coreVertexShader = glCreateShader(GL_VERTEX_SHADER);
    const char* coreVsSrc = vertexSource.c_str();
    glShaderSource(coreVertexShader, 1, &coreVsSrc, NULL);
    glCompileShader(coreVertexShader);

    std::string coreFragmentSource = commonCode + coreMainCode;
    GLuint coreFragmentShader = glCreateShader(GL_FRAGMENT_SHADER);
    const char* coreFsSrc = coreFragmentSource.c_str();
    glShaderSource(coreFragmentShader, 1, &coreFsSrc, NULL);
    glCompileShader(coreFragmentShader);
        ...
        for (auto& batch : batches) {
            ...
            if (vsComplete == GL_TRUE && fsComplete == GL_TRUE) {
                ...
                if (!vsSuccess || !fsSuccess) {
                    if (!vsSuccess) {
                        std::cerr << "Effect shader " << batch.fxid << " vertex compilation failed" << std::endl;
                    }
                    if (!fsSuccess) {
                        std::cerr << "Effect shader " << batch.fxid << " fragment compilation failed" << std::endl;
                        GLint maxLength = 0;
                        glGetShaderiv(batch.fragmentShader, GL_INFO_LOG_LENGTH, &maxLength);
                        if (maxLength > 0) {
                            std::vector<GLchar> infolog(maxLength);
                            glGetShaderInfoLog(batch.fragmentShader, maxLength, &maxLength, infolog.data());
                            std::cerr << "Error: " << infolog.data() << std::endl;
                        }
                    }
    ...
    std::cout << "Phase 4: Starting linking of " << batches.size() << " effect programs..." << std::endl;
    for (auto& batch : batches) {
        if (batch.compileFailed) continue;

        batch.program = glCreateProgram();
        std::cout << "DEBUG Phase 4: Created program " << batch.program << " for fxid=" << batch.fxid << std::endl;
        glAttachShader(batch.program, batch.vertexShader);
        glAttachShader(batch.program, batch.fragmentShader);
        glBindAttribLocation(batch.program, 0, "Position");
        glBindAttribLocation(batch.program, 1, "TexCoord");
    ...
    while (linkedCount < totalToLink) {
        for (auto& batch : batches) {
            ...
            if (linkComplete == GL_TRUE) {
                ...
                if (!linkSuccess) {
                    std::cerr << "Effect shader " << batch.fxid << " linking failed" << std::endl;
                    GLint maxLength = 1024;
                    std::vector<GLchar> infolog(maxLength);
                    glGetProgramInfoLog(batch.program, maxLength, &maxLength, infolog.data());
                    if (maxLength > 0) {
                        std::cerr << "Link error: " << infolog.data() << std::endl;
                    }
                    glDeleteProgram(batch.program);
                    batch.program = 0;
                    batch.linkFailed = true;
                } else {
                    // Success! Store the program
                    EffectShaderPrograms[batch.fxid] = batch.program;
                }
                ...
                // Mark as processed by setting program to 0 (so it won't be checked again)
                batch.program = 0;
    ...
    // PHASE 6: Wait for core shader compilation and link it
    std::cout << "Phase 6: Finalizing core shader (already compiling in parallel)..." << std::endl;
    auto coreStartTime = std::chrono::high_resolution_clock::now();

    // Poll for core shader compilation (already started in Phase 2)
    if (supportsAsync) {
        GLint vsComplete = GL_FALSE, fsComplete = GL_FALSE;
        while (vsComplete == GL_FALSE || fsComplete == GL_FALSE) {
            if (vsComplete == GL_FALSE) {
                glGetShaderiv(coreVertexShader, GL_COMPLETION_STATUS_ARB, &vsComplete);
            }
            if (fsComplete == GL_FALSE) {
                glGetShaderiv(coreFragmentShader, GL_COMPLETION_STATUS_ARB, &fsComplete);
            }
    ...
    GLint vsSuccess, fsSuccess;
    glGetShaderiv(coreVertexShader, GL_COMPILE_STATUS, &vsSuccess);
    glGetShaderiv(coreFragmentShader, GL_COMPILE_STATUS, &fsSuccess);

    if (!vsSuccess || !fsSuccess) {
        if (!vsSuccess) {
            std::cerr << "Core vertex shader compilation failed" << std::endl;
        }
        if (!fsSuccess) {
            std::cerr << "Core fragment shader compilation failed" << std::endl;
            GLint maxLength = 0;
            glGetShaderiv(coreFragmentShader, GL_INFO_LOG_LENGTH, &maxLength);
            if (maxLength > 0) {
                std::vector<GLchar> infolog(maxLength);
                glGetShaderInfoLog(coreFragmentShader, maxLength, &maxLength, infolog.data());
                std::cerr << "Error: " << infolog.data() << std::endl;
            }
        }
        glDeleteShader(coreVertexShader);
        glDeleteShader(coreFragmentShader);
        // Fall back to using set_shader() later
        ShaderProgram = 0;
    } else {
        // Link core shader program
        GLuint coreProgram = glCreateProgram();
        glAttachShader(coreProgram, coreVertexShader);
        glAttachShader(coreProgram, coreFragmentShader);
        glBindAttribLocation(coreProgram, 0, "Position");
        glBindAttribLocation(coreProgram, 1, "TexCoord");
        glLinkProgram(coreProgram);

        // Poll for linking
        if (supportsAsync) {
            GLint linkComplete = GL_FALSE;
            while (linkComplete == GL_FALSE) {
                glGetProgramiv(coreProgram, GL_COMPLETION_STATUS_ARB, &linkComplete);
        ...
        // Check link status
        GLint linkSuccess;
        glGetProgramiv(coreProgram, GL_LINK_STATUS, &linkSuccess);

        if (!linkSuccess) {
            std::cerr << "Core shader linking failed" << std::endl;
            GLint maxLength = 1024;
            std::vector<GLchar> infolog(maxLength);
            glGetProgramInfoLog(coreProgram, maxLength, &maxLength, infolog.data());
            if (maxLength > 0) {
                std::cerr << "Link error: " << infolog.data() << std::endl;
            }
            ...
            // Success! Use the streamlined core shader as the main shader
            ShaderProgram = coreProgram;
            std::cout << "Core shader compiled successfully" << std::endl;
        }
    ...
    auto coreEndTime = std::chrono::high_resolution_clock::now();
    auto coreDuration = std::chrono::duration_cast<std::chrono::milliseconds>(coreEndTime - coreStartTime);

    auto endTime = std::chrono::high_resolution_clock::now();
    auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(endTime - startTime);

    std::cout << "Compiled " << linkedCount << "/" << batches.size() << " effect shaders + core shader in "
              << duration.count() << "ms (core: " << coreDuration.count() << "ms)" << std::endl;
...
void Project::autosave() {
    ...
    for (Bin *bin : binsmain->bins) {
        for (BinElement *binel: bin->elements) {
            if (!binel->autosavejpegsaved && binel->jpegpath != "") {
                return;
            }
        }
    }
...
void Program::socket_client(struct sockaddr_in serv_addr, int opt) {
    ...
    while (1) {
        ...
        if (ret >= 0) {
            // Set socket back to blocking mode for stable communication
#ifdef POSIX
            int flags = fcntl(this->sock, F_GETFL);
            fcntl(this->sock, F_SETFL, flags & ~O_NONBLOCK);
#endif
