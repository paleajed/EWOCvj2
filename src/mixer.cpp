...
void Param::handle(bool smallxpad) {
    ...
    if (this->type != FF_TYPE_BUFFER) {
        ...
        if (this->type == ISFLoader::PARAM_COLOR) {
            ...
            col[1] = this->colvalue[1];
            col[2] = this->colvalue[2];
        	pariscol = true;
    	...
    	if (pariscol)
    	{
    		draw_box(grey, col, this->box->vtxcoords->x1 + pad,
					 ...
					 this->box->vtxcoords->w - pad * 2.0f, this->box->vtxcoords->h * 0.75f, -1);
    	}
        else if (!onoff && this->type != FF_TYPE_OPTION && this->type != FF_TYPE_TEXT && this->type != FF_TYPE_FILE &&
        ...
        } else if (this != mainmix->prepadaptparam && this != mainmix->adaptparam)
        {
	        thisstr = this->name;
        }
        else if (pariscol)
        {
        	...
        	std::sprintf(result, "%02X%02X%02X",
						 std::clamp((int)(col[0] * 255), 0, 255),
						 std::clamp((int)(col[1] * 255), 0, 255),
						 std::clamp((int)(col[2] * 255), 0, 255));
...
Effect::Effect() {
    ...
    this->maskbutton = new Button(false);
    this->maskbutton->toggle = 1;
    this->maskbutton->box->vtxcoords->w = 0.015f;
    this->maskbutton->box->vtxcoords->h = 0.03f;
    this->maskbutton->box->reserved = true;
    this->maskbutton->layer = this->layer;
    this->maskbutton->box->tooltiptitle = "Apply mask to effect ";
    this->maskbutton->box->tooltip = "Leftclick to toggle if the mask influences this effect. ";
    this->maskbutton->box->upvtxtoscr();
    this->editmaskbox = new Boxx;
    this->editmaskbox->vtxcoords->w = 0.015f;
    this->editmaskbox->vtxcoords->h = 0.03f;
    this->editmaskbox->tooltiptitle = "Edit effect maks ";
    this->editmaskbox->tooltip = "Leftclick to mask edit mode for this effect. ";
...
Layer* Mixer::add_layer(std::vector<Layer*> &layers, int pos) {
	...
	for (int c = 0; c < 2; c++)
	{
		for (int d = 0; d < 2; d++)
		{
			if (mainmix->editedmask[c][d])
			{
				if (layers == mainmix->editedmask[c][d]->masks)
				{
					layer->deck = d;
				}
			}
			...
			{
				if (layers == mainmix->editedmaskeff[c][d]->masks)
				{
					layer->deck = d;
				}
			}
...
Layer::Layer(bool comp) {
    ...
    this->laymasked->box->vtxcoords->y1 = 1.0f - mainprogram->layh * 1.5f - 0.44f;
...
Layer::~Layer() {
    ...
    if (!this->dontclosemasks) {
        for (auto lay: this->masks) {
            mainmix->masklayersclose.push_back(lay);
            for (int m = 0; m > 2; m++) {
                for (auto eff: lay->effects[m]) {
                    for (auto efflay: eff->masks) {
                        mainmix->masklayersclose.push_back(efflay);
                    }
                }
            }
        }
    ...
    /*// Clean up parentlay map - remove entries where this layer is the value (parent)
    for (auto it = mainmix->parentlay.begin(); it != mainmix->parentlay.end(); ) {
        if (it->second == this) {
            it = mainmix->parentlay.erase(it);
        } else {
            ++it;
        }
    }
    // Clean up parentlay map - remove entry where this layer is the key (mask)
    mainmix->parentlay.erase(this);

    // Clean up parenteff map similarly reminder
    for (auto it = mainmix->parenteff.begin(); it != mainmix->parenteff.end(); ) {
        if (it->second == this) {
            it = mainmix->parenteff.erase(it);
        } else {
            ++it;
        }
    }
    mainmix->parenteff.erase(this)*/;

    if (!this->dontcloseclips) {
        for (Clip *clip: *this->clips) {
            delete clip;
        }
        //delete this->currclip;   because currclip is reused for new replacement layer
    }
...
Layer* Layer::next() {
    for (int i = 0; i < 4; i++) {
    	std::vector<Layer*> lvecpre = mainmix->editedmask[i / 2][i % 2] ? mainmix->editedmask[i / 2][i % 2]->masks : choose_layers(i % 2);
    	std::vector<Layer*> lvec = mainmix->editedmaskeff[i / 2][i % 2] ? mainmix->editedmaskeff[i / 2][i % 2]->masks : lvecpre;
    	if (std::find(lvec.begin(), lvec.end(), this) != lvec.end()) {
            if (this->pos < lvec.size() - 1) return lvec[this->pos + 1];
        }
    }
	return this;
}
...
void Layer::inhibit() {
	...
	{
		this->olddeckspeed = mainmix->editedmaskeff[this->comp][this->deck]->deckspeed[this->comp][this->deck]->value;
	}
	else if (mainmix->editedmask[this->comp][this->deck])
	{
		this->olddeckspeed = mainmix->editedmask[this->comp][this->deck]->deckspeed[this->comp][this->deck]->value;
	}
	else
	{
...
void Layer::deautomate() {
    ...
    auto allb = loopstation->allbuttons;
    for (Button *but : allb) {
...
void Mixer::begin_mask_hold(Layer* lay) {
    ...
    // If already holding, return the previous saved resources to the pool first
    if (lay->maskloading) {
        if (lay->monholdtex != -1) { mainprogram->add_to_texpool(lay->monholdtex); lay->monholdtex = -1; }
        if (lay->monholdfbo != -1) { mainprogram->add_to_fbopool(lay->monholdfbo); lay->monholdfbo = -1; }
    }

    // Find the last active effect in the render chain (same logic as make_layboxes)
    Effect* lasteff = nullptr;
    for (auto eff : lay->effects[0]) {
        if (eff->onoffbutton->value) lasteff = eff;
    }

    // Null out the last-in-chain fbo/fbotex so onestepfrom spawns a fresh one.
    // The old tex/fbo are saved so the monitor keeps showing pre-transition content.
    if (lasteff) {
        lay->monholdtex = lasteff->fbotex;
        lay->monholdfbo = lasteff->fbo;
        lasteff->fbotex = -1;
        lasteff->fbo = -1;
    } else {
        lay->monholdtex = lay->fbotex;
        lay->monholdfbo = lay->fbo;
        lay->fbotex = -1;
        lay->fbo = -1;
    }

    lay->initdeck = true;
    lay->maskloading = true;
}

bool Mixer::all_masks_loaded(Layer* lay) {
    for (Layer* m : lay->masks) {
        if (!m->filename.empty() && m->changeinit != 2) return false;
        if (!all_masks_loaded(m)) return false;
    }
    for (int n = 0; n < 2; n++) {
        for (Effect* eff : lay->effects[n]) {
            for (Layer* em : eff->masks) {
                if (!em->filename.empty() && em->changeinit != 2) return false;
                if (!all_masks_loaded(em)) return false;
            }
        }
    }
    return true;
...
void Mixer::reconnect_mask_children(Layer* lay) {
    if (lay->masks.empty()) return;
    reconnect_all(lay->masks);
    for (Layer* masklay : lay->masks) {
        reconnect_mask_children(masklay);
        for (int n = 0; n < 2; n++) {
            for (Effect* eff : masklay->effects[n]) {
                reconnect_all(eff->masks);
                for (Layer* submask : eff->masks) {
                    reconnect_mask_children(submask);
                }
            }
        }
    }
...
void make_layboxes(bool post_walk) {
		...
		for (int j = 0; j < mainprogram->nodesmain->pages[i]->nodescomp.size(); j++) {
			...
			if (mainprogram->nodesmain->linked) {
				if (node->type == VIDEO) {
					...
					int lpos = vnode->layer->pos - *scrollpos;
                    ...
                    Effect *lasteffect;
                    if (vnode->layer->maskloading) {
                        bool all_ready = (vnode->layer->changeinit == 2 || vnode->layer->filename.empty());
                        if (all_ready) {
                            all_ready = mainmix->all_masks_loaded(vnode->layer);
                        }
                        if (all_ready && post_walk) {
                            vnode->layer->maskloading = false;
                            vnode->layer->initdeck = false;
                        }
                    }
				...
				if (node->type == VIDEO) {
					if (((VideoNode*)node)->layer) {
                        ...
                        if (vnode->layer->maskloading) {
                            ...
                            if (all_ready && post_walk) {
                                if (vnode->layer->monholdtex != -1) { mainprogram->add_to_texpool(vnode->layer->monholdtex); vnode->layer->monholdtex = -1; }
                                if (vnode->layer->monholdfbo != -1) { mainprogram->add_to_fbopool(vnode->layer->monholdfbo); vnode->layer->monholdfbo = -1; }
                                vnode->layer->maskloading = false;
                                vnode->layer->initdeck = false;
                            }
                        }
	...
	if (mainprogram->nodesmain->linked) {
		...
		for (int j = offs; j < 2 + offs; j++) {
            xoffset = (j % 2) * (1.0f - 0.2f);
			std::vector<Layer*> lvec;
            if (mainmix->editedmaskeff[j / 2][j % 2]) {
                lvec = mainmix->editedmaskeff[j / 2][j % 2]->masks;
            }
            else if (mainmix->editedmask[j / 2][j % 2]) {
                lvec = mainmix->editedmask[j / 2][j % 2]->masks;
            }
            else {
                lvec = mainmix->layers[j];
            }
...
static int decode_audio_packet(Layer *lay, AVPacket* pkt) {
    ...
    if (err2 >= 0 && lay->decframe->nb_samples > 0) {
    	...
    	float fac;
    	if (mainmix->editedmaskeff[!mainprogram->prevmodus][lay->deck]) {
    		fac = mainmix->editedmaskeff[!mainprogram->prevmodus][lay->deck]->deckspeed[!mainprogram->prevmodus][lay->deck]->value;
    	}
    	else if (mainmix->editedmask[!mainprogram->prevmodus][lay->deck]) {
    		fac = mainmix->editedmask[!mainprogram->prevmodus][lay->deck]->deckspeed[!mainprogram->prevmodus][lay->deck]->value;
    	}
    	else {
...
void Layer::get_cpu_frame(int framenr, int prevframe, int errcount)
{
    ...
    if (this->type != ELEM_LIVE) {
        ...
        if (framenr != (int) this->startframe->value) {
            ...
            // Seeking to startframe (beginning of stream) - clear audio queue to sync with video loop
            {
        ...
        { int r = av_read_frame(this->video, this->decpkt); if (r < 0) { if (this->video->pb && this->video->pb->error < 0) this->io_error = true; return; } }
        while (true) {
...
// Decompresses a video frame using Snappy and manages resource allocation for frame data.
    ...
    if (r < 0) {
        if (this->video->pb && this->video->pb->error < 0) this->io_error = true;
        return false;
    }
    ...
    if (isSnappy) {
        ...
        frameDataSize = size - headerl;
    }

    {
        std::lock_guard<std::mutex> lock(this->databuf_mutex);
        if (frameDataSize != this->databufsize) {
            for ( int m = 0; m < 2; m++) {
                if (this->databuf[m]) {
                    free(this->databuf[m]);
                }
                this->databuf[m] = (char *) calloc(frameDataSize, 1);
                if (this->databuf[m] == nullptr) {
                    printf("Can't allocate frame data buffer\n");
                    return false;
                }
            }
            this->databufsize = frameDataSize;
    ...
    int st = -1;
    {
        ...
        if (this->databufready) {
            ...
            }
            {
                ...
                this->decresult->size = frameDataSize;
            }
        }
    }
    av_packet_unref(this->decpkt);
            ...
            if (this->vidformat != 187 && this->vidformat != 188) {
                if (this->io_error) {
                    ...
                    if (now - this->last_reopen_tick > 1000) {
                        this->last_reopen_tick = now;
                        std::string saved_fn = this->filename;
                        int saved_reset = this->reset;
                        this->reset = 0;
                        this->vidopen = true;
                        if (this->thread_vidopen()) {
                            this->io_error = false;
                        } else {
                            this->filename = saved_fn;
                            mainprogram->openerr = false;
                        }
                        this->vidopen = false;
                        this->reset = saved_reset;
                    }
                } else if (this->video && current_frame != current_prevframe) {
...
void Mixer::vidbox_handle() {
    ...
    if (mainprogram->shift || mainprogram->ctrl) return;
    ...
    if (this->pos >= *scrollpos && this->pos < *scrollpos + 3 - this->ismask) {
		...
		if (mainmix->mode == 0 && mainprogram->nodesmain->linked) {
            ...
            if (box->in()) {
                ...
                if (mainprogram->dropfiles.size()) {
                    ...
                    if (this->ismask) {
                        mainprogram->pathto = "OPENFILESMASK";
                    }
                    else {
                ...
                if (std::find(mainmix->currlays[!mainprogram->prevmodus].begin(), mainmix->currlays[!mainprogram->prevmodus].end(), this) != mainmix->currlays[!mainprogram->prevmodus].end()) {
                    ...
                    if (this->closebox->in()) {
                        render_text("x", red, this->closebox->vtxcoords->x1, this->closebox->vtxcoords->y1, 0.00075f,
                                    0.0012f);
                        if (mainprogram->leftmousedown) {
                            this->closing = true;
                            mainprogram->leftmousedown = false;
                        ...
                        if (this->closing && mainprogram->leftmouse) {
                            bool ism = this->ismask;
                            auto lrs = *this->layers;
                        ...
                        render_text("x", white, this->closebox->vtxcoords->x1, this->closebox->vtxcoords->y1, 0.00075f,
                                    0.0012f);
                    }
                    if (this->addbox->in()) {
                        render_text("+", green, this->addbox->vtxcoords->x1, this->addbox->vtxcoords->y1, 0.00075f,
                                    0.0012f);
                        if (mainprogram->leftmousedown) {
                            this->adding = true;
                            mainprogram->leftmousedown = false;
                        ...
                        if (this->adding && mainprogram->leftmouse) {
                            ...
                            if (this->ismask) {
                                ...
                                nlay = mainmix->add_layer(*this->layers, this->pos + 1);
                            }
                            if (nlay->pos > *scrollpos +
                                            2) {   // layer at end->scroll
                                *scrollpos = nlay->pos - 2;
                            }
                            this->adding = false;
                        }
                    } else {
                        render_text("+", white, this->addbox->vtxcoords->x1, this->addbox->vtxcoords->y1, 0.00075f,
                                    0.0012f);
                    }
                    if (this->pos == *scrollpos) {
                    ...
                    if (1)
                    {
	                    if (this->mutebut->box->in()) {
	                    	render_text("M", alphablue, this->mutebut->box->vtxcoords->x1 + 0.0078f,
										this->mutebut->box->vtxcoords->y1 + 0.0078f, 0.0006, 0.001);
	                    	if (mainprogram->leftmousedown) {
	                    		this->muting = true;
	                    		mainprogram->leftmousedown = false;
	                    	}
	                    	...
	                    	render_text("M", alphawhite, this->mutebut->box->vtxcoords->x1 + 0.0078f,
										this->mutebut->box->vtxcoords->y1 + 0.0078f, 0.0006, 0.001);
	                    }
                    }
                    this->solobut->box->vtxcoords->x1 = this->mutebut->box->vtxcoords->x1 + 0.03f;
                    ...
                    if (this->queuebut->box->in()) {
                        register_triangle_draw(black, alphablue, this->queuebut->box->vtxcoords->x1,
                                               this->queuebut->box->vtxcoords->y1, this->queuebut->box->vtxcoords->w,
                                               this->queuebut->box->vtxcoords->h, UP, CLOSED);
                        register_triangle_draw(black, black, this->queuebut->box->vtxcoords->x1,
                                               this->queuebut->box->vtxcoords->y1, this->queuebut->box->vtxcoords->w,
                                               this->queuebut->box->vtxcoords->h, UP, OPEN);
                        if (mainprogram->leftmousedown) {
                        ...
                        if (this->mousequeue && mainprogram->leftmouse) {
                            ...
                            if (this->queueing) mainprogram->queueing = true;
                                ...
                                bool found = false;
                                for (int i = 0; i < 2; i++) {
                                	std::vector<Layer*>& lvecpre = mainmix->editedmask[this->comp][i] ? mainmix->editedmask[this->comp][i]->masks : choose_layers(i);
                                	std::vector<Layer*>& lvec = mainmix->editedmaskeff[this->comp][i] ? mainmix->editedmaskeff[this->comp][i]->masks : lvecpre;
                                    for (int j = 0; j < lvec.size(); j++) {
                                        if (lvec[j]->queueing) {
                                            found = true;
                                            break;
                        ...
                        register_triangle_draw(black, alphawhite, this->queuebut->box->vtxcoords->x1,
                                               this->queuebut->box->vtxcoords->y1, this->queuebut->box->vtxcoords->w,
                                               this->queuebut->box->vtxcoords->h, UP, CLOSED);
                        register_triangle_draw(black, black, this->queuebut->box->vtxcoords->x1,
                                               this->queuebut->box->vtxcoords->y1, this->queuebut->box->vtxcoords->w,
                    ...
                    if (this->beatdetbut->box->in()) {
                        ...
                        if (this->beatdetting && mainprogram->leftmouse) {
                            if (!mainprogram->menuondisplay) {
                                ...
                                mainprogram->beatmenu->menux = mainprogram->mx;
                                mainprogram->beatmenu->menuy = mainprogram->my;
                                mainmix->mouselayer = this;
                                mainmix->mousedeck = this->deck;
                                mainprogram->menuactivation = false;
                            }
                        ...
                        render_text("B", alphawhite, this->beatdetbut->box->vtxcoords->x1 + 0.0078f,
                                    this->beatdetbut->box->vtxcoords->y1 + 0.0078f, 0.0006, 0.001);
                    }
                }
			}
			else {
                if (mainprogram->prevmodus == !this->comp && this->deck == 1 && (this->pos == *scrollpos + 2 || this->pos == this->layers->size() - 1)) {
                ...
                if (0 <= numonscreen && numonscreen <= 2) {
                    if (mainprogram->xvtxtoscr(
                            mainprogram->numw + this->deck * 1.0f + numonscreen * mainprogram->layw) <
                        mainprogram->mx && mainprogram->mx < this->deck * (glob->w / 2.0f) + glob->w / 2.0f) {
        ...
        if (!this->queueing) {
            ...
            for (int i = 0; i < evec.size(); i++) {
                ...
                if (!deleted) {
                    if (eff->masks.size()) {
                        ...
                        eff->maskbutton->box->upvtxtoscr();
                        mainprogram->handle_button(eff->maskbutton);
                        render_text("M", white, eff->maskbutton->box->vtxcoords->x1 + 0.004f,
                                    eff->maskbutton->box->vtxcoords->y1 + 0.004f,
                                    0.00045f, 0.00075f);

                        if (eff->maskbutton->toggled()) {
                    ...
                    if (!(this->clips->size() > 1 && this->keepeffbut->value == 0)) {
                        eff->editmaskbox->vtxcoords->x1 =
                                eff->box->vtxcoords->x1 + eff->box->vtxcoords->w - eff->delbox->vtxcoords->w -
                                0.015f;
                        eff->editmaskbox->vtxcoords->y1 =
                                eff->box->vtxcoords->y1 + eff->box->vtxcoords->h / 2.0f - eff->editmaskbox->vtxcoords->h;
                        eff->editmaskbox->upvtxtoscr();
                        draw_box(lightgrey, blue, eff->editmaskbox, -1);
                        render_text("E", white, eff->editmaskbox->vtxcoords->x1 + 0.005f,
                                    eff->editmaskbox->vtxcoords->y1 + 0.004f,
                                    0.00045f, 0.00075f);
                        if (eff->editmaskbox->in()) {
                            if (mainprogram->leftmouse) {
                                if (eff->masks.empty()) {
            ...
            if (!mainprogram->queueing) {
                ...
                if (inbetween && !mainprogram->drageff) { //true when mouse inbetween effect stack entries
                    inbetween = false;
                	mainprogram->frontbatch = true;
            ...
            if (mainprogram->drageff) {
                int pos = -1;
            	int cnt = this->numrows + this->blendnode->numrows;
                for (int j = 0; j < evec.size() + 1; j++) {
                    // calculate dragged effect temporary position pos when mouse between
                    // limits under and upper
                	int under, upper;
                	if (cnt >= this->effscroll[cat])
                	{
                		if (j == evec.size())
                		{
                			under = evec[j - 1]->box->scrcoords->y1 + evec[j - 1]->box->scrcoords->h * 0.5f;
                			upper = evec[j - 1]->box->scrcoords->y1 + evec[j - 1]->box->scrcoords->h * 1.5f;
                		}
                		else {
                			under = evec[j]->box->scrcoords->y1 - evec[j]->box->scrcoords->h * 0.5f;
                			upper = evec[j]->box->scrcoords->y1 + evec[j]->box->scrcoords->h * 0.5f;
                		}
                	}

                    if (mainprogram->my > under && mainprogram->my < upper) {
                        std::string name = mainprogram->drageff->get_namestring();
                        draw_box(white, darkred1, mainprogram->drageff->box->vtxcoords->x1,
                                 1.0f - mainprogram->yscrtovtx(under), mainprogram->drageff->box->vtxcoords->w,
                                 mainprogram->drageff->box->vtxcoords->h, -1);
                        render_text(name, white, mainprogram->drageff->box->vtxcoords->x1 + 0.015f,
                                    1.0f - mainprogram->yscrtovtx(under) + 0.075f - 0.045f, 0.00045f,
                                    0.00075f);
                        pos = j;
                        break;
                    }
                	...
                	{
                		cnt += evec[j]->numrows;
                		if (cnt > this->effscroll[cat] + mainprogram->efflines)
                		{
                			break;
                		}
                	}
            ...
            // Draw "masked" toggle
            if (this->masks.size()) {
                // layer mask on/off toggle button
            ...
            this->editmaskbox->vtxcoords->x1 = this->mixbox->vtxcoords->x1 - this->laymasked->box->vtxcoords->w;
            this->editmaskbox->upvtxtoscr();
        	draw_box(this->editmaskbox, -1);
            if (this->editmaskbox->in())
            {
        		draw_box(white, lightblue, this->editmaskbox, -1);
	            if (mainprogram->leftmouse) {
	            	if (this->masks.empty()) {
        	...
        	render_text("EM", white, this->editmaskbox->vtxcoords->x1 + this->editmaskbox->vtxcoords->w / 4.0f, this->editmaskbox->vtxcoords->y1 + this->editmaskbox->vtxcoords->h / 4.0f, 0.0006f, 0.0010f);
	...
	if (mainprogram->nodesmain->linked && this->currlay[!mainprogram->prevmodus]) {
		...
		if (lay->vidmoving && mainprogram->layerdragmenu->state != 2) {
			...
			if (mainprogram->lmover) {
				...
				if (!mainprogram->binsroom) {
					if (mainprogram->prevmodus) {
                        std::vector<Layer*> &lvec0pre = mainmix->editedmask[0][0] ? mainmix->editedmask[0][0]->masks : this->layers[0];
                        std::vector<Layer*> &lvec1pre = mainmix->editedmask[0][1] ? mainmix->editedmask[0][1]->masks : this->layers[1];
                        std::vector<Layer*> &lvec0 = mainmix->editedmaskeff[0][0] ? mainmix->editedmaskeff[0][0]->masks : lvec0pre;
                        std::vector<Layer*> &lvec1 = mainmix->editedmaskeff[0][1] ? mainmix->editedmaskeff[0][1]->masks : lvec1pre;
						if (mainprogram->mx < glob->w / 2.0f) ret = lay->exchange(lvec, lvec0, 0);
						else ret = lay->exchange(lvec, lvec1, 1);
					}
					else {
                        std::vector<Layer*> &lvec2pre = mainmix->editedmask[1][0] ? mainmix->editedmask[1][0]->masks : this->layers[2];
                        std::vector<Layer*> &lvec3pre = mainmix->editedmask[1][1] ? mainmix->editedmask[1][1]->masks : this->layers[3];
                        std::vector<Layer*> &lvec2 = mainmix->editedmaskeff[1][0] ? mainmix->editedmaskeff[1][0]->masks : lvec2pre;
                        std::vector<Layer*> &lvec3 = mainmix->editedmaskeff[1][1] ? mainmix->editedmaskeff[1][1]->masks : lvec3pre;
						if (mainprogram->mx < glob->w / 2.0f) ret = lay->exchange(lvec, lvec2, 0);
						else ret = lay->exchange(lvec, lvec3, 1);
					}
				}
			}
		}
...
void ShelfElement::kill_clayers() {
    for (auto& [key, state] : this->stack_states) {
        ...
        state.clayers.clear();
        state.done = false;
        for (auto& [lay, snap] : state.mask_snapshots) {
            for (Layer* m : snap.masks) mainmix->masklayersclose.push_back(m);
        }
        state.mask_snapshots.clear();
    }
}

/*static*/ void* ShelfElement::stack_key(int comp, int deck) {
    if (mainmix->editedmaskeff[comp][deck]) return mainmix->editedmaskeff[comp][deck];
    if (mainmix->editedmask[comp][deck])    return mainmix->editedmask[comp][deck];
    return nullptr;
}

ShelfElement::StackState& ShelfElement::get_state(void* key) {
    return stack_states[key];
}
...
void Mixer::set_prevshelfdragelem_layers(ShelfElement *elem, Layer* lay) {
	void* key = ShelfElement::stack_key(!mainprogram->prevmodus, lay->deck);
    ...
    if (elem) {
        ...
        state.done = true;*/
    	std::vector<Layer*> &lvec1pre = mainmix->editedmask[!mainprogram->prevmodus][mainmix->mousedeck] ? mainmix->editedmask[!mainprogram->prevmodus][mainmix->mousedeck]->masks : mainmix->newlrs[!mainprogram->prevmodus * 2 + mainmix->mousedeck];
    	std::vector<Layer*> &lvec1 = mainmix->editedmaskeff[!mainprogram->prevmodus][mainmix->mousedeck] ? mainmix->editedmaskeff[!mainprogram->prevmodus][mainmix->mousedeck]->masks : lvec1pre;
    	if (lvec1.empty()) {
    		if (mainmix->editedmaskeff[!mainprogram->prevmodus][mainmix->mousedeck]) {
    			lvec1 = mainmix->editedmaskeff[!mainprogram->prevmodus][mainmix->mousedeck]->masks;
    		} else if (mainmix->editedmask[!mainprogram->prevmodus][mainmix->mousedeck]) {
    			lvec1 = mainmix->editedmask[!mainprogram->prevmodus][mainmix->mousedeck]->masks;
    		}
    	}
    	bool cond = true;
		if (elem) {
			if (elem->type == ELEM_DECK) {
                ...
                if (elem->launchtype == 1) {
					...
					for (int i = 0; i < lvec1.size(); i++) {
						...
						if (mainmix->editedmaskeff[!mainprogram->prevmodus][lay->deck])
						{
							lvec1[i]->olddeckspeed = mainmix->editedmaskeff[!mainprogram->prevmodus][lay->deck]->deckspeed[!mainprogram->prevmodus][lay->deck]->value;
						}
						else if (mainmix->editedmask[!mainprogram->prevmodus][lay->deck])
						{
							lvec1[i]->olddeckspeed = mainmix->editedmask[!mainprogram->prevmodus][lay->deck]->deckspeed[!mainprogram->prevmodus][lay->deck]->value;
						}
						else
						{
                ...
                if (elem->launchtype == 1) {
                    for (int m = 0; m < 2; m++) {
                        std::vector<Layer*> &lvec2pre = mainmix->editedmask[!mainprogram->prevmodus][m] ? mainmix->editedmask[!mainprogram->prevmodus][m]->masks : choose_layers(m);
                    ...
                    return;
 					...
 					for (int m = 0; m < 2; m++) {
						...
						for (int i = 0; i < lvec2.size(); i++) {
							if (mainmix->editedmaskeff[!mainprogram->prevmodus][lay->deck])
							{
								lvec2[i]->olddeckspeed = mainmix->editedmaskeff[!mainprogram->prevmodus][lay->deck]->deckspeed[!mainprogram->prevmodus][lay->deck]->value;
							}
							else if (mainmix->editedmask[!mainprogram->prevmodus][lay->deck])
							{
								lvec2[i]->olddeckspeed = mainmix->editedmask[!mainprogram->prevmodus][lay->deck]->deckspeed[!mainprogram->prevmodus][lay->deck]->value;
							}
							else
							{
								lvec2[i]->olddeckspeed = mainmix->deckspeed[!mainprogram->prevmodus][lay->deck]->value;
							}
                            lvec2[i]->tagged = true;
                            state.nblayers.push_back(lvec2[i]);
                            elem->set_nbclayers(lvec2[i]);
 						}
					}
                    return;
		...
		if (cond) {
			...
			if (elem->launchtype == 2) {
                clean(state.nblayers);
                lay->tagged = true;
				if (mainmix->editedmaskeff[!mainprogram->prevmodus][lay->deck])
				{
					lay->olddeckspeed = mainmix->editedmaskeff[!mainprogram->prevmodus][lay->deck]->deckspeed[!mainprogram->prevmodus][lay->deck]->value;
				}
				else if (mainmix->editedmask[!mainprogram->prevmodus][lay->deck])
				{
					lay->olddeckspeed = mainmix->editedmask[!mainprogram->prevmodus][lay->deck]->deckspeed[!mainprogram->prevmodus][lay->deck]->value;
				}
				else
				{
...
void Mixer::open_dragbinel(Layer *thislay, int i) {
    ...
    std::vector<Layer*> &lvecpre = mainmix->editedmask[newlay->comp][newlay->deck] ? mainmix->editedmask[newlay->comp][newlay->deck]->masks : choose_layers(newlay->deck);
    std::vector<Layer*> &lvec = mainmix->editedmaskeff[newlay->comp][newlay->deck] ? mainmix->editedmaskeff[newlay->comp][newlay->deck]->masks : lvecpre;
...
void Mixer::reconnect_all(std::vector<Layer*> &layers) {
	    	...
	    	if (layers[j]->pos > 0 && living) {
	    		if (layers[j]->blendnode->in) {
	    			...
	    			if (layers[j]->lasteffnode[1]->out.size())
	    			{
	    				...
	    				mainprogram->nodesmain->currpage->connect_nodes(layers[j]->blendnode->in,layers[j]->lasteffnode[1]->out[0]);
	    			}
	    			layers[j]->lasteffnode[0]->out.clear();
	    			layers[j]->blendnode->in2 = nullptr;
	    		}
    ...
    if (!living) {
        ...
        for (int i = 0; i < 4; i++) {
            if (&layers == &mainmix->layers[i]) {
            	if (mainprogram->nodesmain->mixnodes[i / 2][i % 2]->in)
            	{
            		mainprogram->nodesmain->mixnodes[i / 2][i % 2]->in->out.clear();
            	}
            	mainprogram->nodesmain->mixnodes[i / 2][i % 2]->in = nullptr;
                mainprogram->nodesmain->currpage->connect_nodes(mainmix->emptylayer[i / 2][i % 2]->node,
                                                                mainprogram->nodesmain->mixnodes[i / 2][i % 2]);
            }
        }
    }
...
bool Layer::exchange(std::vector<Layer*>& slayers, std::vector<Layer*>& dlayers, bool deck) {
	...
	bool sw = (slayers[0]->comp != dlayers[0]->comp);
    ...
    for (int i = 0; i < size; i++) {
        ...
        if (dropin || (box->scrcoords->y1 < mainprogram->my + box->scrcoords->h && mainprogram->my < box->scrcoords->y1)) {
            if ((box->scrcoords->x1 + mainprogram->xvtxtoscr(0.075f) < mainprogram->mx && mainprogram->mx < box->scrcoords->x1 + box->scrcoords->w - mainprogram->xvtxtoscr(0.075f)))
            {
            	...
            	auto parent = inlay->parentlayer;
            	std::vector<Layer*> masks;
            	if (imask)
            	{
            		masks = mainmix->editedmask[inlay->comp][inlay->deck]->masks;
            		masks = mainmix->editedmaskeff[inlay->comp][inlay->deck] ? mainmix->editedmaskeff[inlay->comp][inlay->deck]->masks : masks;
            	}
            	...
            	{
            		slayers[this->pos]->parentlayer = this->parentlayer;
            	}
            	slayers[this->pos]->masks = this->masks;
            	...
            	dlayers[i]->blendnode->layer = this;
            	...
            	if (!mainmix->editedmask[dlayers[i]->comp][dlayers[i]->deck])
            	{
            		dlayers[i]->parentlayer = dlayers[i];
            	}
            	else
            	{
            		dlayers[i]->parentlayer = parent;
            	}
            	if (mainmix->editedmaskeff[dlayers[i]->comp][dlayers[i]->deck])
            	{
            		mainmix->editedmaskeff[dlayers[i]->comp][dlayers[i]->deck]->masks = masks;
            	}
            	else if (mainmix->editedmask[dlayers[i]->comp][dlayers[i]->deck])
            	{
             		mainmix->editedmask[dlayers[i]->comp][dlayers[i]->deck]->masks = masks;
           		}
                ...
                if (pos + endx != 0) {
                    if (nextlay) {
                        ...
                        nextlay->blendnode->layer = nextlay;
                ...
                dlayers[pos + endx]->ismask = ism;
                dlayers[pos + endx]->parentlayer = parent;
                dlayers[pos + endx]->masks = masks;
                int *scrollpos = nullptr;
                ...
                if (dlayers[pos + endx]->pos == *scrollpos + 3) {
                ...
                if (slayers.empty())
                {
                	...
                	if (sw) {
             			int pos = std::find(mainmix->currlays[mainprogram->prevmodus].begin(), mainmix->currlays[mainprogram->prevmodus].end(), this) - mainmix->currlays[mainprogram->prevmodus].begin();
               			mainmix->currlay[mainprogram->prevmodus] = slayers[0];
                		mainmix->currlays[mainprogram->prevmodus][pos] = slayers[0];
                	}
                }

            	texlay = dlayers[pos + endx];
     	...
     	if (texlay)
    	{
    		texlay->comp = !mainprogram->prevmodus;
   			if (sw)
    		{
   				mainprogram->add_to_texpool(texlay->fbotex);
   				GLuint tex = set_texes(texlay->fbotex, texlay->fbo, mainprogram->ow[texlay->comp], mainprogram->oh[texlay->comp]);
   				texlay->fbotex = tex;
   				mainprogram->add_to_texpool(texlay->tempfbotex);
   				tex = set_texes(texlay->tempfbotex, texlay->tempfbo, mainprogram->ow[texlay->comp], mainprogram->oh[texlay->comp]);
   				texlay->tempfbotex = tex;
   				mainprogram->add_to_texpool(texlay->blendnode->fbotex);
   				tex = set_texes(texlay->blendnode->fbotex, texlay->blendnode->fbo, mainprogram->ow[texlay->comp], mainprogram->oh[texlay->comp]);
   				texlay->blendnode->fbotex = tex;
   				for (int m = 0; m < 2; m++)
    			{
    				for (auto eff : texlay->effects[m])
    				{
    					mainprogram->add_to_texpool(eff->fbotex);
    					GLuint tex = set_texes(eff->fbotex, eff->fbo, mainprogram->ow[texlay->comp], mainprogram->oh[texlay->comp]);
    					eff->fbotex = tex;
    					mainprogram->add_to_texpool(eff->tempfbotex);
    					tex = set_texes(eff->tempfbotex, eff->tempfbo, mainprogram->ow[texlay->comp], mainprogram->oh[texlay->comp]);
    					eff->tempfbotex = tex;
    				}
    			}
    		}
...
void Mixer::set_values(Layer *clay, Layer *lay, bool doclips) {
	...
	clay->masked = lay->masked;
	clay->deckspeed[lay->comp][lay->deck]->value = lay->deckspeed[lay->comp][lay->deck]->value;
    clay->masks = lay->masks;
}
...
void Mixer::copy_to_comp(bool deckA, bool deckB, bool comp) {
    ...
    if (deckA) {
        while (!exists(mainprogram->temppath + "copytocompdeckA.deck")) {
            Sleep(5);
        }
        mainmix->mousedeck = 0;
    ...
    if (deckB) {
        while (!exists(mainprogram->temppath + "copytocompdeckB.deck")) {
            Sleep(5);
        }
        mainmix->mousedeck = 1;
	...
	for (LoopStationElement *elem: loopstation->odelems) {
		...
		elem->eventpos = bunowlpst->elements[elem->pos]->eventpos;
	}
...
void Mixer::open_state(std::string path, bool undo) {
    ...
    while (safegetline(rfile, istring)) {
        ...
        if (istring == "MAINSCENE1") {
        	...
        	{
        		mainprogram->prevmodus = true;
        		mainmix->open_mix(result + "_8.file", true);
        		mainprogram->prevmodus = false;
        		mainmix->open_mix(result + "_9.file", true);
        	}
    ...
    if (result != "") {
    	if (version == 1) {
    		mainprogram->shelves[0][0]->open(result + "_0.file", undo);
    		mainprogram->shelves[1][0]->open(result + "_1.file", undo);
    	}
    	else
    	{
    		mainprogram->shelves[0][0]->open(result + "_0.file", undo);
    		mainprogram->shelves[1][0]->open(result + "_1.file", undo);
    		mainprogram->shelves[0][1]->open(result + "_2.file", undo);
    		mainprogram->shelves[1][1]->open(result + "_3.file", undo);
    		mainprogram->shelves[0][2]->open(result + "_4.file", undo);
    		mainprogram->shelves[1][2]->open(result + "_5.file", undo);
    		mainprogram->shelves[0][3]->open(result + "_6.file", undo);
    		mainprogram->shelves[1][3]->open(result + "_7.file", undo);
    	}
    }
	...
	if (version > 1)
	{
		mainprogram->remove(result + "_2.file");
		mainprogram->remove(result + "_3.file");
		mainprogram->remove(result + "_4.file");
		mainprogram->remove(result + "_5.file");
		mainprogram->remove(result + "_6.file");
		mainprogram->remove(result + "_7.file");
	}
}
...
void Mixer::save_state(std::string path, bool autosave, bool undo) {
	...
	filestoadd.push_back(mainprogram->temppath + "tempB0.shelf");
	mainprogram->shelves[0][1]->save(mainprogram->temppath + "tempA1.shelf", undo, false);
	filestoadd.push_back(mainprogram->temppath + "tempA1.shelf");
	mainprogram->shelves[1][1]->save(mainprogram->temppath + "tempB1.shelf", undo, false);
	filestoadd.push_back(mainprogram->temppath + "tempB1.shelf");
	mainprogram->shelves[0][2]->save(mainprogram->temppath + "tempA2.shelf", undo, false);
	filestoadd.push_back(mainprogram->temppath + "tempA2.shelf");
	mainprogram->shelves[1][2]->save(mainprogram->temppath + "tempB2.shelf", undo, false);
	filestoadd.push_back(mainprogram->temppath + "tempB2.shelf");
	mainprogram->shelves[0][3]->save(mainprogram->temppath + "tempA3.shelf", undo, false);
	filestoadd.push_back(mainprogram->temppath + "tempA3.shelf");
	mainprogram->shelves[1][3]->save(mainprogram->temppath + "tempB3.shelf", undo, false);
	filestoadd.push_back(mainprogram->temppath + "tempB3.shelf");
...
void Mixer::open_mix(const std::string path, bool alive, bool loadevents) {
    ...
    for (int m = 0; m < 2; m++) {
        bool comp = !mainprogram->prevmodus;
        auto &lvecpre = mainmix->editedmask[comp][m] ? mainmix->editedmask[comp][m]->masks : (m == 0 ? lvec0 : lvec1);
        auto &lvec = mainmix->editedmaskeff[comp][m] ? mainmix->editedmaskeff[comp][m]->masks : lvecpre;
        for (Layer *lay : lvec) lay->keepmaskbut->value = 0;
    }
    ...
    while (safegetline(rfile, istring)) {
        ...
        if (istring == "LAYERSA") {
            ...
            this->editedmask[!mainprogram->prevmodus][0] = nullptr;
            this->editedmaskeff[!mainprogram->prevmodus][0] = nullptr;
            ...
            this->editedmask[!mainprogram->prevmodus][1] = nullptr;
            this->editedmaskeff[!mainprogram->prevmodus][1] = nullptr;
    ...
    mainmix->currlays[!mainprogram->prevmodus].clear();
    std::vector<Layer*> A_layers = mainmix->editedmask[!mainprogram->prevmodus][0] ? mainmix->editedmask[!mainprogram->prevmodus][0]->masks : mainmix->newlrs[!mainprogram->prevmodus * 2];
    A_layers = mainmix->editedmaskeff[!mainprogram->prevmodus][0] ? mainmix->editedmaskeff[!mainprogram->prevmodus][0]->masks : A_layers;
    for (int pos : cls1) {
        if (pos < A_layers.size()) {
            mainmix->currlays[!mainprogram->prevmodus].push_back(A_layers[pos]);
        }
    }

    std::vector<Layer*> B_layers = mainmix->editedmask[!mainprogram->prevmodus][1] ? mainmix->editedmask[!mainprogram->prevmodus][1]->masks : mainmix->newlrs[!mainprogram->prevmodus * 2 + 1];
    B_layers = mainmix->editedmaskeff[!mainprogram->prevmodus][1] ? mainmix->editedmaskeff[!mainprogram->prevmodus][1]->masks : B_layers;
    for (int pos : cls2) {
        if (pos < B_layers.size()) {
            mainmix->currlays[!mainprogram->prevmodus].push_back(B_layers[pos]);
        }
    }

    if (cldeck == 0) {
        if (clpos < A_layers.size()) {
            mainmix->currlay[!mainprogram->prevmodus] = A_layers[clpos];
        }
    } else {
        if (clpos < B_layers.size()) {
...
void Mixer::open_deck(const std::string path, bool alive, bool loadevents, int copycomp) {
    ...
    {
        bool comp = !mainprogram->prevmodus;
        auto &lvecpre = mainmix->editedmask[comp][mainmix->mousedeck] ? mainmix->editedmask[comp][mainmix->mousedeck]->masks : choose_layers(mainmix->mousedeck);
        auto &lvec = mainmix->editedmaskeff[comp][mainmix->mousedeck] ? mainmix->editedmaskeff[comp][mainmix->mousedeck]->masks : lvecpre;
        for (Layer *lay : lvec) lay->keepmaskbut->value = 0;
    }
...
bool Layer::thread_vidopen() {
    // Unref packets/frames before freeing contexts to prevent heap corruption
    // from dangling buffer references into the old codec/format contexts
    if (this->decpkt) av_packet_unref(this->decpkt);
    if (this->decpktseek) av_packet_unref(this->decpktseek);
    if (this->audiopkt_dedicated) av_packet_unref(this->audiopkt_dedicated);
    if (this->decframe) av_frame_unref(this->decframe);
    if (this->resampled_audioframe) av_frame_unref(this->resampled_audioframe);
...
void Layer::playaudio() {
    ...
    ALuint buffHolder[48]; // An array to hold catch the unqueued buffers
    bool done = false;
    std::list<ALuint> bufferQueue; // A quick and dirty queue of buffer objects
    ALuint temp[48];
    alGenBuffers(48, &temp[0]);
    ...
    while (this->audioplaying) {
        ...
        // Check for audio restart flag to clear OpenAL buffers on video loop
...
bool Layer::progress(bool comp, bool alive, bool doclips) {
    ...
    if (this->type != ELEM_LIVE) {
        if (!this->vidopen) {
        	...
        	float fac = 0.0f;
        	if (mainmix->editedmaskeff[comp][this->deck]) {
        		fac = mainmix->editedmaskeff[comp][this->deck]->deckspeed[comp][this->deck]->value;
        	}
        	else if (mainmix->editedmask[comp][this->deck]) {
        		fac = mainmix->editedmask[comp][this->deck]->deckspeed[comp][this->deck]->value;
        	}
        	else {
        		fac = mainmix->deckspeed[comp][this->deck]->value;
        	}
            if (!alive) fac = 1.0f;
            if (this->clonesetnr != -1) {
                std::unordered_set<Layer*>::iterator it;
                for (it = mainmix->clonesets[this->clonesetnr]->begin(); it != mainmix->clonesets[this->clonesetnr]->end(); it++) {
                    Layer* l = *it;
                    if (l->deck == !this->deck) {
                    	if (mainmix->editedmaskeff[comp][l->deck]) {
                    		fac *= mainmix->editedmaskeff[comp][l->deck]->deckspeed[comp][l->deck]->value;
                    	}
                    	else if (mainmix->editedmask[comp][l->deck]) {
                    		fac *= mainmix->editedmask[comp][l->deck]->deckspeed[comp][l->deck]->value;
                    	}
                    	else {
                    		fac *= mainmix->deckspeed[comp][l->deck]->value;
                    	}
                        break;
                    }
                }
...
void Layer::open_files_layers() {
    ...
    if (mainmix->addbefore) {
        std::vector<Layer*> &lvecpre = mainmix->editedmask[!mainprogram->prevmodus][mainmix->mouselayer->deck] ? mainmix->editedmask[!mainprogram->prevmodus][mainmix->mouselayer->deck]->masks : choose_layers(mainmix->mouselayer->deck);
        std::vector<Layer*> &lvec = mainmix->editedmaskeff[!mainprogram->prevmodus][mainmix->mouselayer->deck] ? mainmix->editedmaskeff[!mainprogram->prevmodus][mainmix->mouselayer->deck]->masks : lvecpre;
        if (!mainmix->addlay) {
    ...
    std::vector<Layer*> &lvecpre = mainmix->editedmask[!mainprogram->prevmodus][deck] ? mainmix->editedmask[!mainprogram->prevmodus][deck]->masks : choose_layers(deck);
    std::vector<Layer*> &lvec = mainmix->editedmaskeff[!mainprogram->prevmodus][deck] ? mainmix->editedmaskeff[!mainprogram->prevmodus][deck]->masks : lvecpre;
...
Layer* Mixer::read_layers(std::istream &rfile, const std::string result, std::vector<Layer*> &to_layers, bool deck, bool isdeck, int type, bool doclips, bool concat, bool load, bool loadevents, bool save, bool keepeff, bool masks, bool keepmask) {
			...
			while (safegetline(rfile, istring)) {
				...
				if (doclips) {
                    ...
                    if (istring == "CLIPLAYER") {
                        ...
                        Layer *cliplay = mainmix->read_layers(rfile, result, cliplayers, 0, false, 0, 0, 1, 0, 0, 0, false, true);
    ...
    if (!masks && to_layers.size()) {
        ...
        if (to_layers[0] == bulrs[mainprogram->prevmodus][deck][0]) {
            mainmix->swapmap[pos].clear();
            lrs = bulrs[mainprogram->prevmodus][deck];
            mainmix->newlrs[pos] = layers;
            tempmap = &mainmix->swapmap[pos];
        }
...
std::vector<std::string> Mixer::write_layer(Layer* lay, std::ostream& wfile, bool doclips, bool dojpeg) {
	...
	if (lay->filename != "") {
        ...
        wfile << "WIDTH\n";
        wfile << std::to_string(mainprogram->ow[!mainprogram->prevmodus]);
        wfile << "\n";
        wfile << "HEIGHT\n";
        wfile << std::to_string(mainprogram->oh[!mainprogram->prevmodus]);
        wfile << "\n";
    }
    if (lay->node) {
    ...
    if (lay->masks.size()) {
        ...
        for (auto masklay: lay->masks) {
            std::vector<std::string> jpegpaths2;
            jpegpaths2 = this->write_layer(masklay, wfile, true, false);
            for (auto jstr: jpegpaths2) {
                jpegpaths.push_back(jstr);
            }
        }
        wfile << "ENDOFMASKS\n";
    }
	...
	wfile << "LAYDECKSPEED\n";
	wfile << std::to_string(lay->deckspeed[lay->comp][lay->deck]->value);
	wfile << "\n";
	...
	for (int j = 0; j < lay->effects[0].size(); j++) {
        ...
        if (eff->masks.size())
        {
        	...
        	wfile << "EFFDECKSPEED\n";
        	wfile << std::to_string(eff->deckspeed[lay->comp][lay->deck]->value);
        	wfile << "\n";
        }
	}
	wfile << "ENDOFEFFECTS\n";
    ...
    mainprogram->recundo = false;
    /*if (mainprogram->undowaiting == 1)
    {
	    mainprogram->undowaiting = 0;
    }*/
...
void Mixer::record_video(std::string reccod) {
    ...
    if (this->reckind) {
    	...
    	if (config.layerFilename.substr(config.layerFilename.size() - 8, 4) == "_hap")
    	{
    		path = find_unused_filename(basename(config.layerFilename.substr(0, config.layerFilename.size() - 8)), config.recdir, "_REC_hap.mov");
    	}
    	else
    	{
...
void Mixer::handle_clips() {
	...
	for (int i = 0; i < 2; i++) {
        std::vector<Layer*>& layspre = mainmix->editedmask[!mainprogram->prevmodus][i] ? mainmix->editedmask[!mainprogram->prevmodus][i]->masks : choose_layers(i);
        std::vector<Layer*>& lays = mainmix->editedmaskeff[!mainprogram->prevmodus][i] ? mainmix->editedmaskeff[!mainprogram->prevmodus][i]->masks : layspre;
    ...
    for (int deck = 0; deck < 2; deck++) {
        if (mainmix->editedmaskeff[!mainprogram->prevmodus][deck]) {
            clip_inside_test(mainmix->editedmaskeff[!mainprogram->prevmodus][deck]->masks, deck);
        } else if (mainmix->editedmask[!mainprogram->prevmodus][deck]) {
            clip_inside_test(mainmix->editedmask[!mainprogram->prevmodus][deck]->masks, deck);
        } else {
            clip_inside_test(this->layers[!mainprogram->prevmodus * 2 + deck], deck);
        }
    }
	...
	if (this->clips->size() > 1) {
		...
		if (lay->currclip->type == ELEM_LAYER) {
            ...
            /*if (!this->ismask) {
                ...
                if (mainmix->neweffmasks[this->comp * 2 + this->deck].size()) {
                    mainmix->parenteff.erase(mainmix->neweffmasks[this->comp * 2 + this->deck][0]);
                }
                mainmix->newmasks[this->comp * 2 + this->deck].clear();
		...
		void* key = ShelfElement::stack_key(!mainprogram->prevmodus, this->deck);
		bool brk = false;
		for (int d = 0; d < 2; d++)
		{
			for (int b = 0; b < 4; b++)
			{
				for (int e = 0; e < 16; e++)
				{
					auto& state = mainprogram->shelves[d][b]->elements[e]->get_state(key);
					for (int i = 0; i < state.clayers.size(); i++)
					{
						if (this->layerId == state.clayers[i]->layerId)
						{
							state.clayers[i]->tagged = false;
							state.clayers[i] = lay;
							state.layIds[i] = lay->layerId;
							lay->tagged = true;
							brk = true;
							break;
						}
...
Layer* Layer::transfer(bool clones, bool dontdeleffs, bool exchange, bool image) {
    ...
    if (mainmix->directtransfer) {
        ...
        if (!mainmix->busyopen) {
            if (mainmix->editedmaskeff[this->comp][this->deck]) {
                ...
                newl = &mainmix->neweffmasks[this->comp * 2 + this->deck];
                if (mainmix->swapmaskeffmap[!mainprogram->prevmodus * 2 + this->deck].size()) {
                    mainmix->openmap.clear();
                    return this;
                }
            }
            else if (mainmix->editedmask[this->comp][this->deck]) {
                tempmap = &mainmix->swapmaskmap[!mainprogram->prevmodus * 2 + this->deck];
                newl = &mainmix->newmasks[this->comp * 2 + this->deck];
                if (mainmix->swapmaskmap[!mainprogram->prevmodus * 2 + this->deck].size()) {
                    mainmix->openmap.clear();
                    return this;
                }
            }
                ...
                newl = &mainmix->newlrs[this->comp * 2 + this->deck];
        ...
        auto lvec = mainmix->editedmask[this->comp][this->deck] ? mainmix->editedmask[this->comp][this->deck]->masks : mainmix->layers[!mainprogram->prevmodus * 2 + this->deck];
        lvec = mainmix->editedmaskeff[this->comp][this->deck] ? mainmix->editedmaskeff[this->comp][this->deck]->masks : lvec;
        for (auto l2: lvec) {
            if (l2 == this) {
                ...
                if (mainmix->editedmaskeff[this->comp][this->deck]) {
                    ...
                    mainmix->parentlay[(*(newl))[0]] = mainmix->editedmask[this->comp][this->deck];
                }
    ...
    if (this->keepmaskbut->value)
    {
    	...
    	if (this->tagged) {
    	    // this layer is going to cache (clayers/nblayers); give lay independent clone-copies
    	    // of the masks so the cached originals on 'this' are not shared with the new foreground.
    	    Layer* prevMouseLayer = mainmix->mouselayer;
    	    mainmix->mouselayer = this;  // copy_mask_layer clone path uses mouselayer->iw/ih
    	    lay->masks.clear();
    	    lay->masked = this->masked;
    	    for (Layer* srcMask : this->masks) {
    	        mainmix->copy_mask_layer(srcMask, lay->masks, lay, nullptr, false);
    	    }
    	    if (this->keepeffbut->value || dontdeleffs) {
    	        for (int m = 0; m < 2; m++) {
    	            for (int j = 0; j < (int)this->effects[m].size() && j < (int)lay->effects[m].size(); j++) {
    	                lay->effects[m][j]->masks.clear();
    	                lay->effects[m][j]->masked = this->effects[m][j]->masked;
    	                for (Layer* srcMask : this->effects[m][j]->masks) {
    	                    mainmix->copy_mask_layer(srcMask, lay->effects[m][j]->masks, lay, lay->effects[m][j], false);
    	                }
    	            }
    	        }
    	    }
    	    mainmix->mouselayer = prevMouseLayer;
    	} else {
...
void Scene::switch_to(bool dotempmap) {
    std::vector<std::vector<Layer*>> *tempmap;
    tempmap = &mainmix->swapmap[this->deck + 2];
    if (tempmap->size()) {
        return;
    }
    ...
    if (1) {
        ...
        for (LoopStationElement *elem: lpc->elements) {
            // copy all event related info and the events themselves
...
void Mixer::reload_tagged_elems(ShelfElement *elem, bool deck, Layer *singlelay) {
    void* key = ShelfElement::stack_key(!mainprogram->prevmodus, deck);
    elem->get_state(key).done = false;
    std::vector<Layer*> lvec = mainmix->editedmask[!mainprogram->prevmodus][deck] ? mainmix->editedmask[!mainprogram->prevmodus][deck]->masks : choose_layers(deck);
    lvec = mainmix->editedmaskeff[!mainprogram->prevmodus][deck] ? mainmix->editedmaskeff[!mainprogram->prevmodus][deck]->masks : lvec;
    for (Layer *lay : lvec) {
        if (lay->tagged) {
...
void Mixer::set_layers(ShelfElement  *elem, bool deck) {
    ...
    if (elem->launchtype < 2) {
        if (state.clayers.size()) {
            ...
            for (Layer *lay : lrs) {
                if (lay == this->currlay[!mainprogram->prevmodus]) {
                    this->currlay[!mainprogram->prevmodus] = state.clayers[std::min(lay->pos, (const int)(state.clayers.size() - 1))];
                }
                for (int i = 0; i < this->currlays[!mainprogram->prevmodus].size(); ++i) {
                    if (lay == this->currlays[!mainprogram->prevmodus][i]) {
                        this->currlays[!mainprogram->prevmodus][i] = state.clayers[std::min(lay->pos, (const int)(state.clayers.size() - 1))];
                    }
                }
                lay->dontclosemasks = true;
                lay->dontcloseclips = true;
            ...
            state.mixlrs[deck].clear();
            for (int i = 0; i < state.clayers.size(); i++) {
                Layer *lay = state.clayers[i];
                if (lay->deck != deck) continue;
                state.mixlrs[deck].push_back(lay);
            }
            lrs = state.mixlrs[deck];
            mainmix->scenes[deck][mainmix->currscene[deck]]->scrollpos = state.scrollpos[deck];
            for (auto lay : lrs) {
                lay->layers = &lrs;
                begin_mask_hold(lay);
            }
        ...
        if (state.nblayers.size()) {
        	auto &lrspre = mainmix->editedmask[!mainprogram->prevmodus][deck] ? mainmix->editedmask[!mainprogram->prevmodus][deck]->masks : this->layers[!mainprogram->prevmodus * 2 + deck];
        	auto &lrs = mainmix->editedmaskeff[!mainprogram->prevmodus][deck] ? mainmix->editedmaskeff[!mainprogram->prevmodus][deck]->masks : lrspre;
        	for (Layer *lay : lrs) {
                if (lay == this->currlay[!mainprogram->prevmodus]) {
                    this->currlay[!mainprogram->prevmodus] = state.nblayers[std::min(lay->pos, (const int)(state.nblayers.size() - 1))];
                }
                for (int i = 0; i < this->currlays[!mainprogram->prevmodus].size(); ++i) {
                    if (lay == this->currlays[!mainprogram->prevmodus][i]) {
                        this->currlays[!mainprogram->prevmodus][i] = state.nblayers[std::min(lay->pos, (const int)(state.nblayers.size() - 1))];
                    }
                }
                lay->dontcloseclips = true;
                lay->close();
...
void Mixer::set_frame(ShelfElement *elem, Layer *lay) {
    ...
    if (elem->launchtype == 1) {
        if (state.clayers.size()) {
            lay->frame = state.clayers[0]->frame.load();
            lay->playbut->value = state.clayers[0]->bouncebut->value;
            lay->revbut->value = state.clayers[0]->bouncebut->value;
            lay->bouncebut->value = state.clayers[0]->bouncebut->value;
        }
    } else if (elem->launchtype == 2) {
        if (state.nblayers.size()) {
            lay->frame = state.nblayers[0]->frame.load();
            lay->playbut->value = state.nblayers[0]->bouncebut->value;
            lay->revbut->value = state.nblayers[0]->bouncebut->value;
            lay->bouncebut->value = state.nblayers[0]->bouncebut->value;
        }
    }
}
