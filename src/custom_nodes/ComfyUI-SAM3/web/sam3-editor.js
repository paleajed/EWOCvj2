...
class SAM3EditorDialog extends ComfyDialog {
	static instance = null;

	static getInstance() {
		if(!SAM3EditorDialog.instance) {
			SAM3EditorDialog.instance = new SAM3EditorDialog();
		}

		return SAM3EditorDialog.instance;
	}
	...
	setlayout(imgCanvas, maskCanvas, pointsCanvas) {
		...
		var cancelButton = this.createRightButton("Cancel", () => {
				document.removeEventListener("mouseup", SAM3EditorDialog.handleMouseUp);
				document.removeEventListener("keydown", SAM3EditorDialog.handleKeyDown);
				self.close();
			});
	...
	show() {
		...
		this.message_box = $el("p", ["Please wait a moment while the SAM3 model and the image are being loaded."]);
	...
	setEventHandler(targetCanvas) {
		...
		document.addEventListener('keydown', SAM3EditorDialog.handleKeyDown);
	}

	static handleKeyDown(event) {
		const self = SAM3EditorDialog.instance;
	...
	async detect(self) {
		...
		const response = await api.fetchApi('/sam3/detect', {
			method: 'POST',
...
app.registerExtension({
	name: "Comfy.SAM3.Editor",
	init(app) {
		const callback =
			function () {
				let dlg = SAM3EditorDialog.getInstance();
				dlg.show();
			};

		const context_predicate = () => ComfyApp.clipspace && ComfyApp.clipspace.imgs && ComfyApp.clipspace.imgs.length > 0
		ClipspaceDialog.registerButton("SAM3 Detector", context_predicate, callback);
	},
