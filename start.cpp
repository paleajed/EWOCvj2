...
{
	//empty temp dir
	boost::filesystem::path path_to_remove(temppath);
...
float render_text(std::string text, float *textc, float x, float y, float sx, float sy, bool smflag) {
	...
	if (prepare) {
		...
		FT_Set_Pixel_Sizes(face, 0, (int)(sy * 32000.0f * h / 1346.0f)); //
		x = -1.0f;
		y = 1.0f;
...
void calc_texture(Layer *lay, bool comp, bool alive) {
	...
	if (!comp) {
		...
		fac = (w / h) / (mainprogram->ow / mainprogram->oh);  //aspect ratio correction
	}
	glDisable(GL_BLEND);
						...
						if (mainprogram->leftmousedown) {
							...
							if (mainprogram->dragbinel) {
								...
								if (mainprogram->dragbinel->path.rfind(".layer") != std::string::npos) {
									...
									std::string newpath = temppath + "shelf_" + base.substr(9, std::string::npos);
...
Param::handle() {
	...
	draw_box(green, green, this->box->vtxcoords->x1 + this->box->vtxcoords->w * ((this->value - this->range[0]) / (this->range[1] - this->range[0])) - tf(0.00078f), this->box->vtxcoords->y1, 2.0f / w, this->box->vtxcoords->h, -1);
}
				...
				if (mainprogram->leftmouse and !mainmix->moving and !mainprogram->menuondisplay) {
					if (i != 0) {
						...
						if (numboxes == mainmix->numboxesA) {
							...
							mainmix->mousedeck = 0;
							mainmix->save_deck(temppath + "tempdeck_xch.deck");
							mainmix->open_deck(temppath + "tempdeck_A" + std::to_string(i) + comp + ".deck", 0);
							...
							mainmix->mousedeck = 1;
							mainmix->save_deck(temppath + "tempdeck_xch.deck");
							mainmix->open_deck(temppath + "tempdeck_B" + std::to_string(i) + comp + ".deck", 0);
...
void the_loop() {
	...
	if (mainprogram->binsscreen) {
		...
		if (k == 0 and mainprogram->menuset == 1) {
			...
			mainmix->mousedeck = 0;
			mainmix->open_deck(mainprogram->movingdeck->path, 1);
		}
			...
			mainprogram->binsscreen = false;
			mainmix->open_mix(mainprogram->movingmix->path.c_str());
		}
				...
				for (int i = 0; i < 6; i++) {
					...
					if (mainprogram->currbinel and lay->vidmoving and mainprogram->leftmouse) {
						...
						if (lay->vidmoving) {
							...
							int count = 0;
							while (1) {
								mainprogram->currbinel->path = temppath + mainprogram->currbin->name + "_" + name + ".layer";
								if (!exists(mainprogram->currbinel->path)) {
									mainmix->save_layerfile(mainprogram->currbinel->path, lay, 1);
									break;
								}
								count++;
								name = remove_version(name) + "_" + std::to_string(count);
					...
					if (mainprogram->currbinel and (mainprogram->inserting > -1) and mainprogram->leftmouse) {
						...
						if (!mainprogram->movingstruct) {
							...
							int count1 = 0;
							while (1) {
								if (mainprogram->inserting == 2) path = temppath + name + "_" + name + ".ewoc";
								else path = temppath + name + "_" + name + ".deck";
								if (!exists(path)) {
									int size = max(((int)((mainprogram->inserttexes[0].size() - 1) / 3) * 3 + 3), ((int)((mainprogram->inserttexes[1].size() - 1) / 3) * 3 + 3));
									for (int d = startdeck; d < enddeck; d++) {
										...
										for (int k = 0; k < mainprogram->inserttexes[d].size(); k++) {
											...
											int count = 0;
											while (1) {
												deckbinel->path = temppath + mainprogram->currbin->name + "_" + name + ".layer";
		...
		for (int i = 0; i < 2; i++) {
			...
			boxlayer.vtxcoords->w = 3.0f / size * mainmix->layw;
			boxlayer.vtxcoords->h = 0.05f;
			for (int j = 0; j < lvec.size(); j++) {
				if (mainmix->scrollpos[i] <= j and j < mainmix->scrollpos[i]+ 3) {
					boxlayer.vtxcoords->x1 = -1.0f + mainmix->numw + i + (j + mainmix->scrollpos[i]) * (mainmix->layw * 3.0f / size); 
					boxlayer.upvtxtoscr();
					draw_box(white, lvec[j]->scrollcol, &boxlayer, -1);
				}
			}
	...
	if (mainprogram->dragdeck) {
		...
		if (!mainprogram->binsscreen) {
			...
			if (boxA.in()) {
				...
				if (mainprogram->leftmouse) {
					mainmix->mousedeck = 0;
					mainmix->open_deck(mainprogram->dragdeck->path, 1);
				}
			}
			else if (boxB.in()) {
	...
	if (mainprogram->dragmix) {
		if (mainprogram->leftmouse or mainprogram->movingstruct) {
			if (sqrt(pow((mainprogram->mx / (w / 2.0f) - 1.0f) * w / h, 2) + pow((h - mainprogram->my) / (h / 2.0f) - 1.25f, 2)) < 0.2f) {
				mainprogram->binsscreen = false;
				mainmix->open_mix(mainprogram->dragmix->path.c_str());
...
std::vector<std::string> Mixer::write_layer(Layer *lay, ostream& wfile, bool doclips) {
	wfile << "POS\n";
...
Mixer::save_layerfile(const std::string &path, Layer *lay, bool doclips) {
    ...
    ofstream outputfile;
	outputfile.open(temppath + "tempconcat", ios::out | ios::binary);
	concat_files(outputfile, str, jpegpaths);
	outputfile.close();
	boost::filesystem::rename(temppath + "tempconcat", str);
}
...
Mixer::save_state(const std::string &path) {
	...
	mainmix->save_mix(temppath + "temp.state1");
	filestoadd.push_back(temppath + "temp.state1.ewoc");
	if (mainmix->compon) {
		mainprogram->preveff = false;
		mainmix->save_mix(temppath + "temp.state2");
...
Mixer::save_mix(const std::string &path) {
    ...
    ofstream outputfile;
	outputfile.open("./tempconcat", ios::out | ios::binary);
	concat_files(outputfile, str, jpegpaths);
	outputfile.close();
...
Mixer::open_layerfile(const std::string &path, Layer *lay, int reset, bool doclips) {
	...
	if (lay->lasteffnode) {
		if (lay->lasteffnode->out.size()) {
			Node *node = lay->lasteffnode->out[0];
			lay->node->out.clear();
			lay->node->out.push_back(node);
		}
			...
			while (getline(rfile, istring)) {
				...
				if (doclips) {
					...
					if (istring == "CLIPLAYER") {
						...
						int count = 0;
						while (1) {
							clip->path = temppath + "cliptemp_" + name + ".layer";
							if (!exists(clip->path)) {
...
GLuint copy_tex(GLuint tex, int tw, int th, bool yflip) {
	...
	float black[4] = {1.0f, 1.0f, 1.0f, 1.0f};
	float facx = (float)tw / w;
	float facy = (float)th / h;
	if (yflip) {
		draw_box(NULL, black, -1.0f, -1.0f + 2.0f * facy, 2.0f * facx, -2.0f * facy, 0.0f, 0.0f, 1.0f, 1.0f, 0, tex, w, h);
	}
	else {
		draw_box(NULL, black, -1.0f, -1.0f + 2.0f * facy, 2.0f * facx, -2.0f * facy, 0.0f, 0.0f, 1.0f, 1.0f, 0, tex, w, h);
	}
...
std::string deconcat_files(const std::string &path) {
	...
	if (concat) {
		...
		int count = 0;
		while (1) {
			outpath = temppath + name + ".mainfile";
			if (!exists(outpath)) {
...
void save_bin(const std::string &path) {
	...
	for (int i = 0; i < 6; i++) {
		for (int j = 0; j < 24; j++) {
			...
			wfile << "\n";
			ELEM_TYPE type = mainprogram->currbin->elements[i * 24 + j]->type;
			if ((type == ELEM_LAYER or type == ELEM_DECK or type == ELEM_MIX) and mainprogram->currbin->elements[i * 24 + j]->path != "") {
				filestoadd.push_back(mainprogram->currbin->elements[i * 24 + j]->path);
			}
			if (mainprogram->currbin->elements[i * 24 + j]->path != "") {
				if (mainprogram->currbin->elements[i * 24 + j]->jpegpath == "") {
					...
					int count = 0;
					while (1) {
						mainprogram->currbin->elements[i * 24 + j]->jpegpath = temppath + mainprogram->currbin->name + "_" + name + ".jpg";
				...
				filestoadd.push_back(mainprogram->currbin->elements[i * 24 + j]->jpegpath);
			}				
	...
	for (int i = 0; i < mainprogram->currbin->decks.size(); i++) {
		...
		wfile << "\n";
		if (mainprogram->currbin->decks[i]->path != "") filestoadd.push_back(mainprogram->currbin->decks[i]->path);
	...
	for (int i = 0; i < mainprogram->currbin->mixes.size(); i++) {
		...
		wfile << "\n";
		if (mainprogram->currbin->mixes[i]->path != "") filestoadd.push_back(mainprogram->currbin->mixes[i]->path);
    ...
    ofstream outputfile;
	outputfile.open(temppath + "tempconcatbin", ios::out | ios::binary);
	std::vector<std::vector<std::string>> filestoadd2;
	filestoadd2.push_back(filestoadd);
	concat_files(outputfile, path, filestoadd2);
	outputfile.close();
	boost::filesystem::rename(temppath + "tempconcatbin", path);
}
...
void save_shelf(const std::string &path) {
    ...
    ofstream outputfile;
	outputfile.open(temppath + "tempconcatshelf", ios::out | ios::binary);
	std::vector<std::vector<std::string>> filestoadd2;
	filestoadd2.push_back(filestoadd);
	concat_files(outputfile, path, filestoadd2);
	outputfile.close();
	boost::filesystem::rename(temppath + "tempconcatshelf", path);
}
...
void save_binslist() {
	...
	wfile << "BINS\n";
	for (int i = 0; i < mainprogram->bins.size(); i++) {
		std::string path = mainprogram->binsdir + mainprogram->bins[i]->name + ".bin";
...
int main(int argc, char* argv[]){
	...
	make_currbin(cb);
	save_binslist(); // updates old bins to latest bin version
	...
	for (int i = 0; i < 4; i++) {
		...
		layA->clips.clear();
		mainmix->mousedeck = 0;
		mainmix->save_deck(temppath + "tempdeck_A" + std::to_string(i + 1) + ".deck");
		mainmix->save_deck(temppath + "tempdeck_A" + std::to_string(i + 1) + "comp.deck");
		mainmix->page[1] = i;
		...
		layB->clips.clear();
		mainmix->mousedeck = 1;
		mainmix->save_deck(temppath + "tempdeck_B" + std::to_string(i + 1) + ".deck");
		mainmix->save_deck(temppath + "tempdeck_B" + std::to_string(i + 1) + "comp.deck");
	...
	mainmix->page[0] = 0;
	mainmix->mousedeck = 0;
	mainmix->open_deck(temppath + "tempdeck_A1.deck", 1);
	mainmix->page[1] = 0;
	mainmix->mousedeck = 1;
	mainmix->open_deck(temppath + "tempdeck_B1.deck", 1);
